...
def _package_parts(path: Path, checkout_root: Path) -> tuple[str, ...]:
    ...
    return tuple(part for part in parts if part != "__pycache__")
...
def _resolve_local_module(
    ...
    *,
    ...
    if package_parts:
        candidates.append(checkout_root.joinpath(*package_parts, *module_parts))
    candidates.append(checkout_root.joinpath(*module_parts))
    candidates.append(current_path.parent.joinpath(*module_parts))
    resolved: list[Path] = []
    ...
    def add_path(candidate: Path) -> None:
        try:
            ...
            return
        package_parts_for_path = relative_parts[:-1]
        for index in range(1, len(package_parts_for_path) + 1):
            initializer = checkout_root.joinpath(
                *package_parts_for_path[:index],
                "__init__.py",
            )
            try:
...
def _imported_module_names(
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            imports.append((module, int(node.level), names))
        elif (
            ...
            imports.append((node.args[0].value, 0, ()))
...
def indexer_dependency_manifest(
    ...
    """Hash the entrypoint and every resolvable local Python import it uses."""
    ...
    try:
        ...
        raise ValueError(
            f"indexer entrypoint {entrypoint} is outside checkout {root}"
        ) from exc
    pending = [entrypoint]
    ...
    while pending:
        ...
        manifest[relative] = sha256(current.read_bytes()).hexdigest()
        try:
            tree = ast.parse(current.read_text(encoding="utf-8"))
        except (OSError, SyntaxError, UnicodeDecodeError):
            continue
        package_parts = _package_parts(current, root)
        for module_name, level, names in _imported_module_names(tree):
            if level:
                base = package_parts[: max(0, len(package_parts) - level + 1)]
                qualified = ".".join((*base, module_name))
            else:
...
def validate_shared_provenance(
    ...
    *,
    ...
    """Validate the receipt fields shared by root and MLX producers."""
