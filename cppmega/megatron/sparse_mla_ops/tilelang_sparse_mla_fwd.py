...
# Original kernel from tile-ai/tilelang/examples/deepseek_v32/sparse_mla_fwd.py.
#
# Key differences from our tilelang_sparse_mla/sparse_mla_fwd.py:
# - LRU kernel cache (thread-safe) to avoid TileLang JIT recompilation
# - Sequence/topk bucketing: pads to multiples so different seq lengths
#   hit the same compiled kernel
...
def sparse_mla_fwd_interface(
    ...
    Internally adds batch dim if needed and pads seq/topk to bucket boundaries
    to avoid TileLang JIT recompilation for every distinct shape.
    """
