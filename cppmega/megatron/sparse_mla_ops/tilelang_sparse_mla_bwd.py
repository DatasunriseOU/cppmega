# ruff: noqa
# Ported from NVIDIA/Megatron-LM PR #3674 (HollowMan6/Megatron-LM:dsa_cp_thd).
# Original kernel from tile-ai/tilelang/examples/deepseek_v32/sparse_mla_bwd.py.
#
# Key differences from our tilelang_sparse_mla/sparse_mla_bwd.py:
# - LRU kernel cache (thread-safe) to avoid TileLang JIT recompilation
# - Sequence/topk bucketing for amortised JIT
# - Uses -1 sentinel for invalid indices (instead of <= max_kv_i causal check)
# - TL_DISABLE_TMA_LOWER: True added to pass_configs
# - Dynamic batch/seq dims (not baked into kernel at compile time)
# - Works with unbatched 3D tensors (adds batch dim internally)
import os
import threading
...
@tilelang.jit(
    out_idx=[-1],
    # GB10 sm_121 has 99 KiB dynamic smem cap (see reference_gb10_bwd_bwd_blocker.md).
    # Aggressive smem merge keeps this kernel under cap on GB10 and is a no-op
    # elsewhere. Enforced by cppmega/megatron/preflight_smem_check.py.
    pass_configs={
