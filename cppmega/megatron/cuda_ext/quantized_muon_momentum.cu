...
// Bumped from 64 -> 128 (2 -> 4 warps/block) to raise SM occupancy on the
// quantized Muon update kernel. With kBlockSize=256, kItemsPerThread becomes 2
// and the indexing pattern (tid + item * kThreads) still tiles the full block
// exactly: items 0,1 cover lanes [0..127] and [128..255] for tid in [0,128).
constexpr int kThreads = 128;
constexpr int kItemsPerThread = kBlockSize / kThreads;
constexpr int kWarpSize = 32;
constexpr int kNumWarps = kThreads / kWarpSize;  // 4
constexpr float kQMax = 127.0f;
// Guard against denormal/tiny absmax values that would produce an inv_scale
// large enough to overflow when multiplied by gradient magnitudes. The threshold
// is well below any value that could realistically arise from bf16/fp16 inputs
// (smallest bf16 normal is ~1.18e-38) yet large enough to prevent kQMax/x from
// exploding to inf. Used only for inv_scale; the un-clamped absmax is still
// stored back to the absmax buffer to preserve next-step accuracy.
constexpr float kAbsmaxEps = 1e-30f;

// Block-wide reduction over kThreads lanes that simultaneously computes the
// max-of-absmax and the sum-of-sumsq. Two-stage: warp shuffle reduce, then a
// final cross-warp shuffle reduce in warp 0. `scratch_max`/`scratch_sum` must
// each point to at least kNumWarps floats of dynamic-size-irrelevant shared
// memory. The result is broadcast to all threads via the scratch buffers
// (lane-0 of warp-0 writes index 0 after the final reduction).
__device__ __forceinline__ void block_reduce_max_sum(
  ...
  // Stage 1: intra-warp reduction via shuffles (no shared memory, no sync).
  #pragma unroll
  ...
  // Stage 2: lane 0 of each warp publishes its warp result.
  if (lane == 0) {
  ...
  // Stage 3: warp 0 reduces the kNumWarps partials. Lanes >= kNumWarps load
  // identity values so the shuffle reduction is correct regardless of mask.
  if (warp_id == 0) {
...
// __launch_bounds__(kThreads, minBlocksPerSM): with kThreads=128 we drop the
// minimum-resident-blocks hint from 4 -> 2. Holding 4 resident 128-thread
// blocks would require 512 threads/SM dedicated to this kernel, which over-
// constrains register allocation on smaller SMs (e.g. SM_75/86) and forces
// register spills. 2 is the natural choice for 128-thread blocks and still
// gives 256 resident threads/SM, matching the previous (64-thread, 4-block)
// configuration.
...
#pragma unroll
  ...
  // Two-stage warp-shuffle reduction; the scratch buffers hold one float per
  // warp (kNumWarps == 4 for kThreads=128), trivially small.
  ...
  // Use the un-clamped new_absmax for absmax[]/next-step state, but a clamped
  // value for inv_scale so that denormal-tiny absmax cannot blow up
  // updated * inv_scale. If absmax sits at/below kAbsmaxEps the block is
  // effectively zero, so emit inv_scale=0 (yielding all-zero quantized codes).
...
// TODO: fuse update + scale via grid_group sync to remove the second kernel
// launch and the global-memory round-trip of the gradient buffer.
