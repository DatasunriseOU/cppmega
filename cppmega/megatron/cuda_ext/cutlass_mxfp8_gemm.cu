...
using CompactScaleAsymmetricAColumnwiseSmemKVectorBTmaEarlyDispatchPolicy =
    cutlass::gemm::collective::MainloopSm120TmaWarpSpecializedBlockScaledCompactScale<
        CollectiveMainloop::DispatchPolicy::Stages,
        CollectiveMainloop::DispatchPolicy::SchedulerPipelineStageCount,
        typename CollectiveMainloop::DispatchPolicy::ClusterShape,
...
using CompactScaleAsymmetricAColumnwiseSmemKVectorBTmaEarlyCollectiveMainloop = cutlass::gemm::collective::CollectiveMma<
    CompactScaleAsymmetricAColumnwiseSmemKVectorBTmaEarlyDispatchPolicy,
    ThreadBlockShape,
    cute::tuple<KernelElementA, KernelElementSF>,
    cute::tuple<StrideA, LayoutSFA>,
    cute::tuple<KernelElementB, KernelElementSF>,
...
using CompactScaleAsymmetricAColumnwiseSmemKVectorBTmaEarlyGemmKernel = cutlass::gemm::kernel::GemmUniversal<
    Shape<int, int, int, int>,
    CompactScaleAsymmetricAColumnwiseSmemKVectorBTmaEarlyCollectiveMainloop,
    CollectiveEpilogue,
    cutlass::gemm::StreamKScheduler>;
using CompactScaleAsymmetricAColumnwiseSmemKVectorBTmaEarlyGemm =
    cutlass::gemm::device::GemmUniversalAdapter<CompactScaleAsymmetricAColumnwiseSmemKVectorBTmaEarlyGemmKernel>;
...
at::Tensor cutlass_mxfp8_tn_gemm_compact_direct_a_col_smem_kvec_b_tma_early_asym_cuda(
    ...
    double beta) {
#if !defined(CUTLASS_ARCH_MMA_SM120_SUPPORTED) && !defined(CUTLASS_ARCH_MMA_SM121_SUPPORTED)
  TORCH_CHECK(false, "CUTLASS SM120/SM121 MXFP8 compact direct A-columnwise-smem k-vector B-TMA-early asymmetric backend was not compiled for this architecture");
#else
  TORCH_CHECK(m64 <= INT_MAX && n64 <= INT_MAX && k64 <= INT_MAX, "M/N/K exceed int");
  int m = static_cast<int>(m64);
  int n = static_cast<int>(n64);
  ...
  TORCH_CHECK(m > 0 && n > 0 && k > 0, "m, n, k must be positive");
  TORCH_CHECK(m % 128 == 0 && n % 128 == 0 && k % 128 == 0,
              "CUTLASS MXFP8 compact direct A-columnwise-smem k-vector B-TMA-early asymmetric backend currently requires M/N/K multiples of 128, got ",
              m, "x", n, "x", k);
  TORCH_CHECK(a_source == kOperandColumnwiseTranspose,
              "A-columnwise-smem k-vector B-TMA-early backend requires A source to be columnwise-transpose");
  TORCH_CHECK(b_source == kOperandRowwise,
              "A-columnwise-smem k-vector B-TMA-early backend requires B source to be rowwise");
  validate_direct_operand(A_u8, SFA_u8, m, k, a_source, a_data_ld, a_scale_ld, "A");
  validate_direct_operand(B_u8, SFB_u8, n, k, b_source, b_data_ld, b_scale_ld, "B");
  if (use_out) {
    CHECK_CUDA(out);
  ...
  return run_compact_direct_gemm_impl<CompactScaleAsymmetricAColumnwiseSmemKVectorBTmaEarlyGemm>(
      A_u8, SFA_u8, B_u8, SFB_u8, m, n, k,
      a_source, a_data_ld, a_scale_ld,
      b_source, b_data_ld, b_scale_ld,
      D, alpha_f, beta_f, stream);
