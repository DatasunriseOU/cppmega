...
at::Tensor run_compact_scale_gemm_impl(
  ...
  // out= tensor (e.g. torch.empty(...) holding NaN) would propagate as
  // NaN * 0 = NaN under IEEE 754.  When beta != 0 we alias C onto D so that
  // accumulate=True reads the existing user values.
...
at::Tensor run_compact_direct_gemm_impl(
  ...
  auto ptr_SFB = reinterpret_cast<KernelElementSF const*>(SFB_u8.data_ptr<uint8_t>());
  auto ptr_D = reinterpret_cast<ElementD*>(D.data_ptr<at::BFloat16>());
  // See run_compact_scale_gemm_impl: skip the C load when beta == 0 to
  // avoid NaN propagation from an uninitialized out= tensor.
  ElementD* ptr_C = (beta == 0.0f) ? nullptr : ptr_D;
