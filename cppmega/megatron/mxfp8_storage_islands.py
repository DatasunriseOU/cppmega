"""Opt-in trainable MXFP8 storage for non-TE parameter islands.
...
while keeping them ``requires_grad=True`` so optimizer in-place updates
re-quantize the primary model storage instead of retaining BF16 parameters.
"""
...
Mxfp8StorageIslandMode = Literal["off", "trainable_mxfp8"]
...
class Mxfp8StorageIslandConfig:
    ...
    ngram_out_proj: bool = True
    structure_table: bool = True
    structure_up_proj: bool = True
    pad_rows: bool = True
...
def mxfp8_storage_island_config_from_env() -> Mxfp8StorageIslandConfig:
    ...
    if mode == "frozen_mxfp8":
        raise ValueError(
            "CPPMEGA_MXFP8_STORAGE_ISLANDS=frozen_mxfp8 is a frozen memory probe, "
            "not a trainable storage path; use trainable_mxfp8"
        )
    if mode not in ("off", "trainable_mxfp8"):
        raise ValueError(
            "CPPMEGA_MXFP8_STORAGE_ISLANDS must be off|trainable_mxfp8; "
            f"got {mode!r}"
        )
    return Mxfp8StorageIslandConfig(
        ...
        ngram_out_proj=_env_bool("CPPMEGA_MXFP8_STORAGE_ISLAND_NGRAM_OUT_PROJ", True),
        structure_table=_env_bool("CPPMEGA_MXFP8_STORAGE_ISLAND_STRUCTURE_TABLE", True),
        structure_up_proj=_env_bool("CPPMEGA_MXFP8_STORAGE_ISLAND_STRUCTURE_UP_PROJ", True),
...
def _copy_parameter_attrs(src: torch.nn.Parameter, dst: torch.nn.Parameter) -> None:
    for key, value in getattr(src, "__dict__", {}).items():
        if key in {"grad", "main_grad", "_cppmega_mxfp8_frozen_storage"}:
            continue
        try:
    ...
    dst._cppmega_mxfp8_trainable_storage = True
    dst.is_emerging_optimizer_fallback_parameter = getattr(
        src, "is_emerging_optimizer_fallback_parameter", False
    )


def _path_allows_row_padding(path: str, parent: object) -> bool:
    """Return whether adding unused rows preserves module semantics."""

    if path == "output_layer.weight":
        return False
    return isinstance(parent, nn.Embedding)


def _path_allows_column_padding(path: str, parent: object) -> bool:
    """Return whether the owner slices padded feature columns back out."""

    return (
        path == "embedding.cppmega_ngram_hash.unified_table.weight"
...
def _padded_source_tensor(
    ...
    *,
    ...
    pad = (-rows) % 32
    if pad and (not pad_rows or not allow_row_padding):
...
def _quantize_param_to_trainable_mxfp8(
    parent: object,
    attr: str,
    path: str,
    *,
    ...
    source, reason, new_shape = _padded_source_tensor(
        ...
        allow_row_padding=_path_allows_row_padding(path, parent),
        allow_column_padding=_path_allows_column_padding(path, parent),
    )
    ...
    q_param = torch.nn.Parameter(q_tensor, requires_grad=current.requires_grad)
    ...
    q_param._cppmega_mxfp8_padded_columns = int(new_shape[1] - old_shape[1])
...
def apply_mxfp8_storage_islands(
    ...
    """Convert selected BF16 storage islands to trainable MXFP8 parameters."""

    if config.mode == "off":
        return ()
    if config.mode != "trainable_mxfp8":
    ...
    for path in selected_mxfp8_storage_island_paths(config):
        ...
        results.append(
            _quantize_param_to_trainable_mxfp8(parent, attr, path, config=config)
        )
    ...
    if converted:
        ...
        _rank0_print(
            "[cppmega] MXFP8 trainable storage islands converted "
    ...
    if skipped:
        details = ", ".join(
            f"{r.path}:{r.reason}:{r.old_nbytes / (1024**2):.3f} MiB"
            for r in skipped
        )
        _rank0_print(
            f"[cppmega] MXFP8 trainable storage islands BF16 remains: {details}",
            stderr=True,
        )
