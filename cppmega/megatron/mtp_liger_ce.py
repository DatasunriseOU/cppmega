...
def patch_mtp_loss_with_liger():
    ...
    Always installs — no env gate.  The native MTP path materialises
    ``[s*b, V]`` logits per depth × MTP_DEPTHS (21 GiB saved at MBS=10
    NAM56R).  Without this patch MBS=10 is impossible, period.

    Raises on any import or install failure — the config is invalid,
    ...
    def _liger_process_mtp_loss(
        ...
        for mtp_layer_number in range(config.mtp_num_layers):
            ...
            # CRITICAL: reduction="mean" only.  reduction="none" hits Liger
            # FLCE bug #968 (element_mul_kernel scalar-broadcast on
            # non-uniform grad_output → silent gradient corruption).  See
            # tests/test_mtp_liger_ce.py and apply_linear_ce_patch.py path
            # #3 for the same workaround on main head.
            #
            # Fold loss_mask into ignore_index so reduction="mean" excludes
            # masked positions natively.  Math: scalar = sum(per_token_CE
            # over valid) / N_valid.  We broadcast back to [b, s] so the
            # downstream MTPLossAutoScaler / loss_mask multiply path is
