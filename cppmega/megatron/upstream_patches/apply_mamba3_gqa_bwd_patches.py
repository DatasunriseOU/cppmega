"""Env-gated applier for Mamba3 MIMO backward GQA reduction support.

This patches the installed ``mamba_ssm`` TileLang source so
``mamba_mimo_bwd_combined`` handles intermediate grouped-query shapes
...
def _patch_text(text: str, path: Path) -> tuple[str, bool]:
    ...
    if _has_partial_markers(text):
        raise RuntimeError(f"{path}: partial Mamba3 GQA bwd patch markers detected")

    original, patched = _blocks_for(path)
    count = text.count(original)
    if count != 1:
        raise RuntimeError(
            f"{path}: expected exactly one unpatched GQA reduction block, found {count}. "
...
def _validate_file(path: Path) -> None:
    ...
    if _has_partial_markers(text):
        raise RuntimeError(f"{path}: partial GQA bwd markers after patch")
...
def _do_patch() -> None:
    ...
    for path in paths:
        ...
        if not changed:
            _validate_file(path)
            print(f"  OK   {path.name}: GQA bwd branch already applied")
            continue
        ...
        _validate_file(path)
        print(f"  DONE {path.name}: GQA bwd branch applied")
...
def rollback() -> None:
    ...
    for path in paths:
        ...
        text = path.read_text()
        if _is_patched_text(text):
            print(f"  SKIP no backup for {path.name}; leaving existing GQA-patched source intact")
            continue

        print(f"  OK   {path.name}: no GQA patch present")


def _is_gqa_patch_applied() -> bool:
    try:
...
def _run_once_with_local_rank_guard(fn, is_done=None) -> None:
    ...
    print(f"[mamba3_gqa_bwd] local_rank={local_rank} rank={rank_env} waiting for local_rank=0")
        ...
        if lock_path.exists():
            with open(lock_path) as lock_fh:
                ...
                try:
                    ...
                    fcntl.flock(lock_fh.fileno(), fcntl.LOCK_UN)
        time.sleep(0.1)
    raise RuntimeError(f"Timed out waiting for local_rank=0 GQA bwd patch. lockfile={lock_path}")
...
def apply_all() -> None:
    ...
    _run_once_with_local_rank_guard(_do_patch, _is_gqa_patch_applied)
