...
class PrecisionProfile:
    ...
    # Explicit profiling lane for non-TE storage islands.  ``trainable_mxfp8``
    # quantizes embedding/output/ngram/structure parameters to TE MXFP8 tensors
    # while keeping them trainable.  The profile must own this knob; env-only
    # enablement makes BF16-storage accounting ambiguous.
    ...
    mxfp8_storage_island_ngram_out_proj: bool = True
    mxfp8_storage_island_structure_table: bool = True
    mxfp8_storage_island_structure_up_proj: bool = True
...
def profile_shell_assignments(profile: RunProfile) -> dict[str, str]:
    ...
    if profile.precision.mxfp8_storage_islands not in ("off", "trainable_mxfp8"):
        raise ValueError(
            "mxfp8_storage_islands must be off|trainable_mxfp8; "
            f"got {profile.precision.mxfp8_storage_islands!r}"
        )
    if (
...
def _add_common_profile_overrides(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        choices=("off", "trainable_mxfp8"),
        default=None,
        help=(
            "Profiling lane for non-TE BF16 islands. trainable_mxfp8 stores "
            "selected embedding/output/ngram/structure parameters as TE MXFP8 "
            "while keeping gradients and optimizer updates enabled."
        ),
    )
    ...
    storage_island_columnwise.add_argument(
        ...
        help=(
            "Also store columnwise MXFP8 payloads for trainable islands. "
            "Disabled by default because embedding/CE forward paths only need "
            "rowwise dequantization."
        ),
    )
    storage_island_columnwise.add_argument(
