...
from cppmega.megatron.upstream_patches import apply_mamba3_grouped_head_bwd_patches as applier
...
def test_grouped_head_patch_text_adds_intermediate_group_branch():
...
def test_grouped_head_patch_text_is_idempotent():
    path = Path("mamba3_mimo_bwd.py")
...
def test_grouped_head_partial_marker_rejected():
    path = Path("mamba3_mimo_bwd.py")
    ...
    try:
        ...
        raise AssertionError("partial grouped-head patch markers should be rejected")


def test_grouped_head_do_patch_and_rollback_from_backups(tmp_path, monkeypatch):
