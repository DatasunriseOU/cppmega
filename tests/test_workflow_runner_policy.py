...
def test_macos_lane_writes_a_pre_python_failure_receipt() -> None:
    ...
    assert 'pre_python_step="Triton and MLX packages are local and pinned"' in macos
...
def test_tilelang_tvm_pin_and_wheel_name_are_consistent() -> None:
    ...
    assert '("tvm_commit", tvm_commit)' in modal_build
    assert '("tvm_ffi_commit", tvm_ffi_commit)' in modal_build
    assert '"tvm_commit": tvm_commit' in modal_build
    assert '"tvm_ffi_commit": tvm_ffi_commit' in modal_build
