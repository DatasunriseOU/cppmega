...
def test_local_gb10_profile_owns_cce_mtp_and_optimizer_defaults():
    ...
    assert env["CPPMEGA_TE_MXFP8_LINEAR_KERNEL_CONTRACT"] == "gemm_ready_v1_dense_only"
...
def test_run_profile_cli_overrides_are_parameters_not_env(capsys, monkeypatch):
    ...
    assert "export CPPMEGA_TE_MXFP8_LINEAR_KERNEL_CONTRACT=gemm_ready_v1_dense_only" in out
    ...
    assert "export CPPMEGA_EXTRA_PYTHONPATH=/tmp/fa:/tmp/te" in out
    assert "export CPPMEGA_TRANSFORMER_ENGINE_SOURCE_ROOT=/tmp/te" in out
    assert "export CPPMEGA_FLASH_ATTN_SOURCE_ROOT=/tmp/fa" in out
...
def test_local_gb10_quarter_mxfp8_defaults_to_measured_te_tn_backend():
    ...
    assert profile.precision.mxfp8_linear_kernel_contract == "gemm_ready_v1_dense_only"
