...
RELATIVE_GLADMANCONTEXT_H = "OSX/libsecurity_apple_csp/lib/gladmanContext.h"
GLADMANCONTEXT_H_SIZE = 2750
GLADMANCONTEXT_H_SHA256 = (
    "5783adfe5bd3af0e40b6f77e6fa850c286a277fa0b3483df643dac9494b9bf5b"
)
GLADMANCONTEXT_SIBLING_PATHS = {
    "OSX/libsecurity_apple_csp/lib/AppleCSPContext.h",
    "OSX/libsecurity_apple_csp/lib/BlockCryptor.h",
    "OSX/libsecurity_apple_csp/lib/desContext.h",
...
def test_apple_security_gladmancontext_libclang_timeout_accepts_header(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    payload = fixture.read_bytes()
    path = tmp_path / "gladmanContext.h"
    path.write_bytes(payload)
    entry = SourceQuarantineEntry(
        project_id="apple-oss-distributions/Security",
        relative_path=RELATIVE_GLADMANCONTEXT_H,
        size_bytes=len(payload),
        sha256=hashlib.sha256(payload).hexdigest(),
        classification="compiler_regression_fixture",
        detected_format="apple_security_libclang_timeout_header",
        reason="Security gladmanContext.h libclang hang",
    )
    _verify_detected_format(path, entry)


def test_apple_security_gladmancontext_contract_rejects_unrelated_standin(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_checked_in_apple_security_gladmancontext_manifest_matches_pinned_fixture() -> None:
    fixture = (
        Path(__file__).resolve().parent
        / "fixtures"
        / "source_quarantine"
    ...
    payload = fixture.read_bytes()
    assert len(payload) == GLADMANCONTEXT_H_SIZE
    assert hashlib.sha256(payload).hexdigest() == GLADMANCONTEXT_H_SHA256
    manifest = json.loads(
        (
            Path(__file__).resolve().parents[1]
            / "configs"
    ...
    entries = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") == RELATIVE_GLADMANCONTEXT_H
    ]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["size_bytes"] == GLADMANCONTEXT_H_SIZE
    assert entry["sha256"] == GLADMANCONTEXT_H_SHA256
    assert entry["detected_format"] == "apple_security_libclang_timeout_header"
    assert entry["project_id"] == "apple-oss-distributions/Security"
    siblings = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") in GLADMANCONTEXT_SIBLING_PATHS
    ]
    assert len(siblings) == 4
    assert entry["sha256"] not in {e["sha256"] for e in siblings}


RELATIVE_RC5CONTEXT_H = "OSX/libsecurity_apple_csp/lib/rc5Context.h"
RC5CONTEXT_H_SIZE = 1817
RC5CONTEXT_H_SHA256 = (
    "a7bb0627768850795cf3ff8f850c7b90ec5235b20704cb9a0ff8a51e6357f8bd"
)
RC5CONTEXT_SIBLING_PATHS = {
...
def test_apple_security_rc5context_libclang_timeout_accepts_header(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    payload = fixture.read_bytes()
    path = tmp_path / "rc5Context.h"
    path.write_bytes(payload)
    entry = SourceQuarantineEntry(
        project_id="apple-oss-distributions/Security",
        relative_path=RELATIVE_RC5CONTEXT_H,
        size_bytes=len(payload),
        sha256=hashlib.sha256(payload).hexdigest(),
        classification="compiler_regression_fixture",
        detected_format="apple_security_libclang_timeout_header",
        reason="Security rc5Context.h libclang hang",
    )
    _verify_detected_format(path, entry)


def test_apple_security_rc5context_contract_rejects_unrelated_standin(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_checked_in_apple_security_rc5context_manifest_matches_pinned_fixture() -> None:
    fixture = (
        Path(__file__).resolve().parent
        / "fixtures"
        / "source_quarantine"
    ...
    payload = fixture.read_bytes()
    assert len(payload) == RC5CONTEXT_H_SIZE
    assert hashlib.sha256(payload).hexdigest() == RC5CONTEXT_H_SHA256
    manifest = json.loads(
        (
            Path(__file__).resolve().parents[1]
            / "configs"
    ...
    entries = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") == RELATIVE_RC5CONTEXT_H
    ]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["size_bytes"] == RC5CONTEXT_H_SIZE
    assert entry["sha256"] == RC5CONTEXT_H_SHA256
    assert entry["detected_format"] == "apple_security_libclang_timeout_header"
    assert entry["project_id"] == "apple-oss-distributions/Security"
    siblings = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") in RC5CONTEXT_SIBLING_PATHS
    ]
...
RELATIVE_NOTIFICATIONS_H = "securityd/src/notifications.h"
NOTIFICATIONS_H_SIZE = 4937
NOTIFICATIONS_H_SHA256 = (
    "cbb828aa0cf41da04a4a6a0fb57e902cbbe4734bae41a831c6757a6695c7f4bf"
)
NOTIFICATIONS_SIBLING_PATHS = {
    "securityd/src/token.h",
    "securityd/src/server.h",
    "securityd/src/tokenaccess.h",
    "securityd/src/process.h",
    "securityd/src/connection.h",
}


def test_apple_security_notifications_libclang_timeout_accepts_header(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    payload = fixture.read_bytes()
    path = tmp_path / "notifications.h"
    path.write_bytes(payload)
    entry = SourceQuarantineEntry(
        project_id="apple-oss-distributions/Security",
        relative_path=RELATIVE_NOTIFICATIONS_H,
        size_bytes=len(payload),
        sha256=hashlib.sha256(payload).hexdigest(),
        classification="compiler_regression_fixture",
        detected_format="apple_security_libclang_timeout_header",
        reason="Security notifications.h libclang hang",
    )
    _verify_detected_format(path, entry)


def test_apple_security_notifications_contract_rejects_unrelated_standin(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_checked_in_apple_security_notifications_manifest_matches_pinned_fixture() -> None:
    fixture = (
        Path(__file__).resolve().parent
        / "fixtures"
        / "source_quarantine"
    ...
    payload = fixture.read_bytes()
    assert len(payload) == NOTIFICATIONS_H_SIZE
    assert hashlib.sha256(payload).hexdigest() == NOTIFICATIONS_H_SHA256
    assert b"#ifndef _H_NOTIFICATIONS" in payload
    assert b"class Listener: public RefCount" in payload
    manifest = json.loads(
        (
            Path(__file__).resolve().parents[1]
            / "configs"
    ...
    entries = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") == RELATIVE_NOTIFICATIONS_H
    ]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["size_bytes"] == NOTIFICATIONS_H_SIZE
    assert entry["sha256"] == NOTIFICATIONS_H_SHA256
    assert entry["detected_format"] == "apple_security_libclang_timeout_header"
    assert entry["project_id"] == "apple-oss-distributions/Security"
    siblings = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") in NOTIFICATIONS_SIBLING_PATHS
    ]
    assert len(siblings) == 5
    assert entry["sha256"] not in {e["sha256"] for e in siblings}


RELATIVE_AGENTQUERY_H = "securityd/src/agentquery.h"
AGENTQUERY_H_SIZE = 7510
AGENTQUERY_H_SHA256 = (
    "109953c2bd06471ac91c225bf87c9c0cbbc06e0dd777097d7d7c9aa648c148f4"
)
AGENTQUERY_SIBLING_PATHS = {
    "securityd/src/notifications.h",
    "securityd/src/authhost.h",
    "securityd/src/server.h",
    "securityd/src/connection.h",
...
def test_apple_security_agentquery_libclang_timeout_accepts_header(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    payload = fixture.read_bytes()
    path = tmp_path / "agentquery.h"
    path.write_bytes(payload)
    entry = SourceQuarantineEntry(
        project_id="apple-oss-distributions/Security",
        relative_path=RELATIVE_AGENTQUERY_H,
        size_bytes=len(payload),
        sha256=hashlib.sha256(payload).hexdigest(),
        classification="compiler_regression_fixture",
        detected_format="apple_security_libclang_timeout_header",
        reason="Security agentquery.h libclang hang",
    )
    _verify_detected_format(path, entry)


def test_apple_security_agentquery_contract_rejects_unrelated_standin(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_checked_in_apple_security_agentquery_manifest_matches_pinned_fixture() -> None:
    fixture = (
        Path(__file__).resolve().parent
        / "fixtures"
        / "source_quarantine"
    ...
    payload = fixture.read_bytes()
    assert len(payload) == AGENTQUERY_H_SIZE
    assert hashlib.sha256(payload).hexdigest() == AGENTQUERY_H_SHA256
    assert b"#ifndef _H_AGENTQUERY" in payload
    assert b"class SecurityAgentXPCQuery : public SecurityAgentXPCConnection" in payload
    manifest = json.loads(
        (
            Path(__file__).resolve().parents[1]
            / "configs"
    ...
    entries = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") == RELATIVE_AGENTQUERY_H
    ]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["size_bytes"] == AGENTQUERY_H_SIZE
    assert entry["sha256"] == AGENTQUERY_H_SHA256
    assert entry["detected_format"] == "apple_security_libclang_timeout_header"
    assert entry["project_id"] == "apple-oss-distributions/Security"
    siblings = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") in AGENTQUERY_SIBLING_PATHS
    ]
    assert len(siblings) == 5
    assert entry["sha256"] not in {e["sha256"] for e in siblings}
...
RELATIVE_SESSION_H = "securityd/src/session.h"
SESSION_H_SIZE = 5026
SESSION_H_SHA256 = (
    "8c3914c29b95dd38b22bb09f24e1d5514cc5286727f8fc472462e3b287581e8c"
)
SESSION_SIBLING_PATHS = {
    "securityd/src/server.h",
    "securityd/src/structure.h",
    "securityd/src/authhost.h",
    "securityd/src/connection.h",
...
def test_apple_security_session_libclang_timeout_accepts_header(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    payload = fixture.read_bytes()
    path = tmp_path / "session.h"
    path.write_bytes(payload)
    entry = SourceQuarantineEntry(
        project_id="apple-oss-distributions/Security",
        relative_path=RELATIVE_SESSION_H,
        size_bytes=len(payload),
        sha256=hashlib.sha256(payload).hexdigest(),
        classification="compiler_regression_fixture",
        detected_format="apple_security_libclang_timeout_header",
        reason="Security session.h libclang hang",
    )
    _verify_detected_format(path, entry)


def test_apple_security_session_contract_rejects_unrelated_standin(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_checked_in_apple_security_session_manifest_matches_pinned_fixture() -> None:
    fixture = (
        Path(__file__).resolve().parent
        / "fixtures"
        / "source_quarantine"
    ...
    payload = fixture.read_bytes()
    assert len(payload) == SESSION_H_SIZE
    assert hashlib.sha256(payload).hexdigest() == SESSION_H_SHA256
    assert b"session - authentication session domains" in payload
    assert b"class Session : public PerSession {" in payload
    manifest = json.loads(
        (
            Path(__file__).resolve().parents[1]
            / "configs"
    ...
    entries = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") == RELATIVE_SESSION_H
    ]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["size_bytes"] == SESSION_H_SIZE
    assert entry["sha256"] == SESSION_H_SHA256
    assert entry["detected_format"] == "apple_security_libclang_timeout_header"
    assert entry["project_id"] == "apple-oss-distributions/Security"
    siblings = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") in SESSION_SIBLING_PATHS
    ]
    assert len(siblings) == 5
    assert entry["sha256"] not in {e["sha256"] for e in siblings}
...
RELATIVE_DATABASE_H = "securityd/src/database.h"
DATABASE_H_SIZE = 8515
DATABASE_H_SHA256 = (
    "8e982492d654680d953a1888ad0043b7b2327e41d2612b43e3b4bb0ddbf73a96"
)
DATABASE_SIBLING_PATHS = {
    "securityd/src/localdatabase.h",
    "securityd/src/tokendatabase.h",
...
def test_apple_security_database_libclang_timeout_accepts_header(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    payload = fixture.read_bytes()
    path = tmp_path / "database.h"
    path.write_bytes(payload)
    entry = SourceQuarantineEntry(
        project_id="apple-oss-distributions/Security",
        relative_path=RELATIVE_DATABASE_H,
        size_bytes=len(payload),
        sha256=hashlib.sha256(payload).hexdigest(),
        classification="compiler_regression_fixture",
        detected_format="apple_security_libclang_timeout_header",
        reason="Security database.h libclang hang",
    )
    _verify_detected_format(path, entry)


def test_apple_security_database_contract_rejects_unrelated_standin(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_checked_in_apple_security_database_manifest_matches_pinned_fixture() -> None:
    fixture = (
        Path(__file__).resolve().parent
        / "fixtures"
        / "source_quarantine"
    ...
    payload = fixture.read_bytes()
    assert len(payload) == DATABASE_H_SIZE
    assert hashlib.sha256(payload).hexdigest() == DATABASE_H_SHA256
    assert b"database - abstract database management" in payload
    assert b"class Database : public PerProcess, public AclSource {" in payload
    manifest = json.loads(
        (
            Path(__file__).resolve().parents[1]
            / "configs"
    ...
    entries = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") == RELATIVE_DATABASE_H
    ]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["size_bytes"] == DATABASE_H_SIZE
    assert entry["sha256"] == DATABASE_H_SHA256
    assert entry["detected_format"] == "apple_security_libclang_timeout_header"
    assert entry["project_id"] == "apple-oss-distributions/Security"
    siblings = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") in DATABASE_SIBLING_PATHS
    ]
    assert len(siblings) == 5
    assert entry["sha256"] not in {e["sha256"] for e in siblings}
...
RELATIVE_AESCSPI_H = "OSX/libsecurity_apple_csp/lib/aescspi.h"
AESCSPI_H_SIZE = 3081
AESCSPI_H_SHA256 = (
    "7ca32d42decde633d333081b9e5f285a35e2a2017157dfcee3ef4a467a854ebb"
)
AESCSPI_SIBLING_PATHS = {
    "OSX/libsecurity_apple_csp/lib/RSA_asymmetric.h",
    "OSX/libsecurity_apple_csp/lib/RSA_DSA_keys.h",
    "OSX/libsecurity_apple_csp/lib/AppleCSPContext.h",
    "OSX/libsecurity_apple_csp/lib/BlockCryptor.h",
    "OSX/libsecurity_apple_csp/lib/DH_keys.h",
...
def test_apple_security_aescspi_libclang_timeout_accepts_header(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    payload = fixture.read_bytes()
    path = tmp_path / "aescspi.h"
    path.write_bytes(payload)
    entry = SourceQuarantineEntry(
        project_id="apple-oss-distributions/Security",
        relative_path=RELATIVE_AESCSPI_H,
        size_bytes=len(payload),
        sha256=hashlib.sha256(payload).hexdigest(),
        classification="compiler_regression_fixture",
        detected_format="apple_security_libclang_timeout_header",
        reason="Security aescspi.h libclang hang",
    )
    _verify_detected_format(path, entry)


def test_apple_security_aescspi_contract_rejects_unrelated_standin(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_checked_in_apple_security_aescspi_manifest_matches_pinned_fixture() -> None:
    fixture = (
        Path(__file__).resolve().parent
        / "fixtures"
        / "source_quarantine"
    ...
    payload = fixture.read_bytes()
    assert len(payload) == AESCSPI_H_SIZE
    assert hashlib.sha256(payload).hexdigest() == AESCSPI_H_SHA256
    assert b"aescspi.h - AES context class" in payload
    assert b"class AESContext : public BlockCryptor {" in payload
    manifest = json.loads(
        (
            Path(__file__).resolve().parents[1]
            / "configs"
    ...
    entries = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") == RELATIVE_AESCSPI_H
    ]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["size_bytes"] == AESCSPI_H_SIZE
    assert entry["sha256"] == AESCSPI_H_SHA256
    assert entry["detected_format"] == "apple_security_libclang_timeout_header"
    assert entry["project_id"] == "apple-oss-distributions/Security"
    siblings = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") in AESCSPI_SIBLING_PATHS
    ]
    assert len(siblings) == 5
    assert entry["sha256"] not in {e["sha256"] for e in siblings}


RELATIVE_BFCONTEXT_H = "OSX/libsecurity_apple_csp/lib/bfContext.h"
BFCONTEXT_H_SIZE = 2419
BFCONTEXT_H_SHA256 = (
    "955d4b42f413cbb3017aaac92cce01a4b031d880eeb82486ea4089ca0a0da6ad"
)
BFCONTEXT_SIBLING_PATHS = {
...
def test_apple_security_bfcontext_libclang_timeout_accepts_header(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    payload = fixture.read_bytes()
    path = tmp_path / "bfContext.h"
    path.write_bytes(payload)
    entry = SourceQuarantineEntry(
        project_id="apple-oss-distributions/Security",
        relative_path=RELATIVE_BFCONTEXT_H,
        size_bytes=len(payload),
        sha256=hashlib.sha256(payload).hexdigest(),
        classification="compiler_regression_fixture",
        detected_format="apple_security_libclang_timeout_header",
        reason="Security bfContext.h libclang hang",
    )
    _verify_detected_format(path, entry)


def test_apple_security_bfcontext_contract_rejects_unrelated_standin(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_checked_in_apple_security_bfcontext_manifest_matches_pinned_fixture() -> None:
    fixture = (
        Path(__file__).resolve().parent
        / "fixtures"
        / "source_quarantine"
    ...
    payload = fixture.read_bytes()
    assert len(payload) == BFCONTEXT_H_SIZE
    assert hashlib.sha256(payload).hexdigest() == BFCONTEXT_H_SHA256
    assert b"bfContext.h - glue between BlockCrytpor and ssleay Blowfish" in payload
    assert b"class BlowfishContext : public BlockCryptor {" in payload
    manifest = json.loads(
        (
            Path(__file__).resolve().parents[1]
            / "configs"
    ...
    entries = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") == RELATIVE_BFCONTEXT_H
    ]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["size_bytes"] == BFCONTEXT_H_SIZE
    assert entry["sha256"] == BFCONTEXT_H_SHA256
    assert entry["detected_format"] == "apple_security_libclang_timeout_header"
    assert entry["project_id"] == "apple-oss-distributions/Security"
    siblings = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") in BFCONTEXT_SIBLING_PATHS
    ]
    assert len(siblings) == 5
    assert entry["sha256"] not in {e["sha256"] for e in siblings}


RELATIVE_CASTCONTEXT_H = "OSX/libsecurity_apple_csp/lib/castContext.h"
CASTCONTEXT_H_SIZE = 3032
CASTCONTEXT_H_SHA256 = (
    "64f1f2d70cf650ae9e09838796fde43212fcf994d4a70145909d89061039f526"
)
CASTCONTEXT_SIBLING_PATHS = {
...
def test_apple_security_castcontext_libclang_timeout_accepts_header(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    payload = fixture.read_bytes()
    path = tmp_path / "castContext.h"
    path.write_bytes(payload)
    entry = SourceQuarantineEntry(
        project_id="apple-oss-distributions/Security",
        relative_path=RELATIVE_CASTCONTEXT_H,
        size_bytes=len(payload),
        sha256=hashlib.sha256(payload).hexdigest(),
        classification="compiler_regression_fixture",
        detected_format="apple_security_libclang_timeout_header",
        reason="Security castContext.h libclang hang",
    )
    _verify_detected_format(path, entry)


def test_apple_security_castcontext_contract_rejects_unrelated_standin(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_checked_in_apple_security_castcontext_manifest_matches_pinned_fixture() -> None:
    fixture = (
        Path(__file__).resolve().parent
        / "fixtures"
        / "source_quarantine"
    ...
    payload = fixture.read_bytes()
    assert len(payload) == CASTCONTEXT_H_SIZE
    assert hashlib.sha256(payload).hexdigest() == CASTCONTEXT_H_SHA256
    assert b"castContext.h - glue between BlockCrytpor and ssleay CAST-128 (CAST5)" in payload
    assert b"class CastContext : public BlockCryptor {" in payload
    manifest = json.loads(
        (
            Path(__file__).resolve().parents[1]
            / "configs"
    ...
    entries = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") == RELATIVE_CASTCONTEXT_H
    ]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["size_bytes"] == CASTCONTEXT_H_SIZE
    assert entry["sha256"] == CASTCONTEXT_H_SHA256
    assert entry["detected_format"] == "apple_security_libclang_timeout_header"
    assert entry["project_id"] == "apple-oss-distributions/Security"
    siblings = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") in CASTCONTEXT_SIBLING_PATHS
    ]
    assert len(siblings) == 5
    assert entry["sha256"] not in {e["sha256"] for e in siblings}
...
RC4CONTEXT_H_SHA256 = (
    "beec4bf2e401e9cddc06c4be4828c2be5c88c27d3017ccebcbb69e94bee1c30e"
)
RC4CONTEXT_SIBLING_PATHS = {
...
def test_apple_security_rc4context_libclang_timeout_accepts_header(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    payload = fixture.read_bytes()
    path = tmp_path / "rc4Context.h"
    path.write_bytes(payload)
    entry = SourceQuarantineEntry(
        project_id="apple-oss-distributions/Security",
        relative_path=RELATIVE_RC4CONTEXT_H,
        size_bytes=len(payload),
        sha256=hashlib.sha256(payload).hexdigest(),
        classification="compiler_regression_fixture",
        detected_format="apple_security_libclang_timeout_header",
        reason="Security rc4Context.h libclang hang",
    )
    _verify_detected_format(path, entry)


def test_apple_security_rc4context_contract_rejects_unrelated_standin(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_checked_in_apple_security_rc4context_manifest_matches_pinned_fixture() -> None:
    fixture = (
        Path(__file__).resolve().parent
        / "fixtures"
        / "source_quarantine"
    ...
    payload = fixture.read_bytes()
    assert len(payload) == RC4CONTEXT_H_SIZE
    assert hashlib.sha256(payload).hexdigest() == RC4CONTEXT_H_SHA256
    assert b"#ifndef _RC4_CONTEXT_H_" in payload
    assert b"class RC4Context : public AppleCSPContext" in payload
    ...
    entries = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") == RELATIVE_RC4CONTEXT_H
    ]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["size_bytes"] == RC4CONTEXT_H_SIZE
    assert entry["sha256"] == RC4CONTEXT_H_SHA256
    assert entry["detected_format"] == "apple_security_libclang_timeout_header"
    assert entry["project_id"] == "apple-oss-distributions/Security"
    siblings = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") in RC4CONTEXT_SIBLING_PATHS
    ]
    assert len(siblings) == 5
    assert entry["sha256"] not in {e["sha256"] for e in siblings}
...
RSA_DSA_KEYS_H_SHA256 = (
    "f8fb3413a41d487c1372ba452855d7d9bfbc5c55c7697ae54fbc02b4cecebf54"
)
RSA_DSA_KEYS_FORMAT = "apple_security_rsa_dsa_keys_macroman_nbsp_libclang_timeout"
RSA_DSA_KEYS_SIBLING_PATHS = {
    "OSX/libsecurity_apple_csp/lib/DH_keys.h",
    "OSX/libsecurity_apple_csp/lib/AppleCSPContext.h",
    "OSX/libsecurity_apple_csp/lib/rc4Context.h",
    "OSX/libsecurity_apple_csp/lib/rc5Context.h",
...
def test_apple_security_rsa_dsa_keys_libclang_timeout_accepts_header(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    payload = fixture.read_bytes()
    path = tmp_path / "RSA_DSA_keys.h"
    path.write_bytes(payload)
    entry = SourceQuarantineEntry(
        project_id="apple-oss-distributions/Security",
        relative_path=RELATIVE_RSA_DSA_KEYS_H,
        size_bytes=len(payload),
        sha256=hashlib.sha256(payload).hexdigest(),
        classification="compiler_regression_fixture",
        detected_format=RSA_DSA_KEYS_FORMAT,
        reason="Security RSA_DSA_keys.h libclang hang",
    )
    _verify_detected_format(path, entry)


def test_apple_security_rsa_dsa_keys_contract_rejects_unrelated_standin(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_apple_security_rsa_dsa_keys_rejects_utf8_nbsp_stripped_copy(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
...
def test_apple_security_rsa_dsa_keys_utf8_header_format_raises(
    tmp_path: Path,
) -> None:
    from tools.clang_indexer.source_quarantine import (
        SourceQuarantineEntry,
    ...
    entry = SourceQuarantineEntry(
        ...
        classification="compiler_regression_fixture",
        detected_format="apple_security_libclang_timeout_header",
        reason="utf-8 format must reject non-UTF-8 RSA_DSA_keys.h",
    )
    with pytest.raises(SourceQuarantineError, match="header is not UTF-8"):
        _verify_detected_format(path, entry)


def test_checked_in_apple_security_rsa_dsa_keys_manifest_matches_pinned_fixture() -> None:
    fixture = (
        Path(__file__).resolve().parent
        / "fixtures"
        / "source_quarantine"
    ...
    payload = fixture.read_bytes()
    assert len(payload) == RSA_DSA_KEYS_H_SIZE
    assert hashlib.sha256(payload).hexdigest() == RSA_DSA_KEYS_H_SHA256
    assert payload.count(b"\xca") == 1
    assert payload[5356] == 0xCA
    manifest = json.loads(
        (
            Path(__file__).resolve().parents[1]
            / "configs"
    ...
    entries = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") == RELATIVE_RSA_DSA_KEYS_H
    ]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["size_bytes"] == RSA_DSA_KEYS_H_SIZE
    assert entry["sha256"] == RSA_DSA_KEYS_H_SHA256
    assert entry["detected_format"] == RSA_DSA_KEYS_FORMAT
    assert entry["project_id"] == "apple-oss-distributions/Security"
    siblings = [
        e
        for e in manifest["entries"]
        if e.get("relative_path") in RSA_DSA_KEYS_SIBLING_PATHS
    ]
    assert len(siblings) == 4
    assert entry["sha256"] not in {e["sha256"] for e in siblings}
