...
class TestBwdParity:
    ...
    def test_bwd_reduced_broadcast_qk_matches_legacy(self, monkeypatch):
        ...
        xf0 = torch.sigmoid(torch.randn(B, S, n_f, device=device, dtype=dtype, generator=g))
        grad_out = torch.randn(B, S, n_v, V, device=device, dtype=dtype, generator=g)

        def run(flag):
            monkeypatch.setenv("CPPMEGA_M2RNN_BWD_REDUCE_BROADCAST_QK", "1" if flag else "0")
        ...
        for name, reduced_grad, legacy_grad in zip(
            ("dq", "dk", "dv", "dW", "dxf"), reduced, legacy
        ):
