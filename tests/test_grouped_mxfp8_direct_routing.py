...
def test_grouped_split_quantize_v1_uses_producer_operands_without_copy(monkeypatch):
    ...
    assert stats["mxfp8_linear_contract_v1_grouped_multi_output_producer"] == 0
    assert stats["mxfp8_grouped_gemm_ready_dgrad"] == 1
    assert stats["mxfp8_grouped_transpose_copy_fallback_dgrad"] == 0
    assert stats["mxfp8_tn_adapter_copy_transpose"] == 0


def test_grouped_split_quantize_multi_output_contract_requires_te_api(monkeypatch):
    shim = _fresh_grouped_shim(
        monkeypatch,
        linear_contract="gemm_ready_v1",
    ...
    monkeypatch.setattr(
        shim,
        "_te_mxfp8_grouped_multi_output_producer_available",
        False,
    )
    monkeypatch.setattr(shim, "_cppmega_mxfp8_fused_producer_missing_api", [])

    quantizers = [_FakeMXFP8Quantizer("q0"), _FakeMXFP8Quantizer("q1")]
    with pytest.raises(RuntimeError, match="grouped multi-output producer"):
        shim._tex.split_quantize(torch.empty((4, 4)), [2, 2], quantizers)

    stats = shim.cppmega_te_mxfp8_bwd_stats
    assert stats["mxfp8_linear_contract_v1_grouped_multi_output_missing_api"] == 1
    assert stats["mxfp8_linear_contract_v1_grouped_split_quantize_producer"] == 0
