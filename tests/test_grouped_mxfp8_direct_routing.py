...
def test_grouped_split_quantize_probe_acceptance_reports_requested_counters():
    probe_path = (
        Path(__file__).resolve().parents[1]
        / "tools"
        / "probes"
        / "te_grouped_split_quantize_profile.py"
    )
    spec = importlib.util.spec_from_file_location(
        "te_grouped_split_quantize_profile_unit",
        probe_path,
    )
    ...
    acceptance = probe._acceptance_counters(
        {
            "bf16_fallback_dgrad": 0,
            "bf16_fallback_wgrad": 0,
            "mxfp8_tn_adapter_copy_transpose": 0,
            "mxfp8_tn_sidecar_registry_peak_bytes": 123,
        },
        {
            "outputs": 16,
            "sidecars": 0,
        ...
        {
            "producer_calls": 16,
            "producer_calls_per_output": 1.0,
            "producer_us_per_output": 7.5,
        },
    )
    ...
    assert acceptance == {
        ...
        "mxfp8_tn_sidecar_registry_peak_bytes": 123,
        "attached_sidecars": 0,
        "attached_sidecar_payload_bytes": 0,
        "producer_calls": 16,
        "producer_calls_per_output": 1.0,
        "producer_us_per_output": 7.5,
        "producer_outputs": 16,
...
def test_grouped_split_quantize_probe_blocked_report_is_machine_readable():
    probe_path = (
        Path(__file__).resolve().parents[1]
        / "tools"
        / "probes"
    ...
    spec = importlib.util.spec_from_file_location(
        "te_grouped_split_quantize_profile_blocked_unit",
        probe_path,
    )
    assert spec is not None
    assert spec.loader is not None
    ...
    profile = probe.GroupedSplitQuantizeProfile(contract="gemm_ready_v1")
    report = probe._blocked_report(
        profile,
        probe.ProbeBlocked(
    ...
    assert report["missing_api"] == ["transformer_engine_torch"]
