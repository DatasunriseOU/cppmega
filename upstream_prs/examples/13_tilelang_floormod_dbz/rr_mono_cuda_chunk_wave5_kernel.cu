...
__global__ void stage2_mono_scan_owner_kernel(
  ...
  const int h = static_cast<int>(pid % H);
  const int b = static_cast<int>(pid / H);
  if (b >= B) {
    return;
  }
...
#pragma unroll
  ...
  for (int idx = tid; idx < kR * P; idx += blockDim.x) {
    dmimo_v_delta[((static_cast<int64_t>(b) * H + h) * kR * P) + idx] = dmimo_s[idx];
  }
}
...
size_t mono_scan_owner_smem_bytes(int P) {
...
void validate_inputs(
  ...
  TORCH_CHECK(chunk_size == kChunk, "Wave 5 scan owner specializes chunk_size=16");
  ...
  TORCH_CHECK(R == kR, "Wave 5 scan owner specializes R=4");
  TORCH_CHECK(N == 64, "Wave 5 scan owner specializes N=64");
  TORCH_CHECK(P >= kPPanel && P % kPPanel == 0, "Wave 5 scan owner requires P to be a positive multiple of 64");
...
void stage2_mono_scan_owner_out(
    const at::Tensor& dout,
    const at::Tensor& q_flat,
    const at::Tensor& k_flat,
    const at::Tensor& v,
  ...
  const int64_t total_programs = static_cast<int64_t>(B) * H;
  const dim3 grid(static_cast<unsigned int>(total_programs));
  const dim3 block(kThreads);
  const size_t smem_bytes = mono_scan_owner_smem_bytes(P);
  auto stream = at::cuda::getCurrentCUDAStream();

  C10_CUDA_CHECK(cudaFuncSetAttribute(
      stage2_mono_scan_owner_kernel,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      static_cast<int>(smem_bytes)));

  stage2_mono_scan_owner_kernel<<<grid, block, smem_bytes, stream>>>(
...
std::vector<at::Tensor> stage2_mono_scan_owner(
    const at::Tensor& dout,
    const at::Tensor& q_flat,
    const at::Tensor& k_flat,
    const at::Tensor& v,
...
py::dict stage2_mono_scan_owner_metadata(const at::Tensor& dout) {
  TORCH_CHECK(dout.is_cuda(), "dout must be CUDA");
  TORCH_CHECK(dout.scalar_type() == at::kBFloat16, "Wave 5 scan metadata requires bf16");
  TORCH_CHECK(dout.dim() == 4, "dout must have shape [B, S, H, P]");

  cudaFuncAttributes attrs{};
  C10_CUDA_CHECK(cudaFuncGetAttributes(&attrs, stage2_mono_scan_owner_kernel));
  ...
  C10_CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &active_blocks, stage2_mono_scan_owner_kernel, kThreads, smem_bytes));
  ...
  out["scan_owner_ctas"] = B * H;
  out["chunks_per_cta"] = nchunks;
  ...
  out["local_reuse"] = "LKQ/masked-LKQ reused across all P64 panels in one chunk; final DMIMO_V accumulated in CTA-local shared memory";
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def(
      "stage2_mono_scan_owner",
      &stage2_mono_scan_owner,
      "Wave 5 scan-owner WMMA state/LKQ/D kernel");
  m.def(
      "stage2_mono_scan_owner_out",
      &stage2_mono_scan_owner_out,
      "Wave 5 scan-owner WMMA state/LKQ/D kernel into preallocated outputs");
  m.def(
      "stage2_mono_scan_owner_metadata",
      &stage2_mono_scan_owner_metadata,
      "Wave 5 scan-owner WMMA state/LKQ/D kernel metadata");
}
