"""Minimal CUDA extension wrapper for the Wave 4 P64-panel WMMA kernel."""
...
def _load_extension() -> Any:
    ...
    name = f"rr_mono_cuda_chunk_wave4_ext_t{threads}"
    if suffix:
        name = f"{name}_{suffix}"
    os.environ.setdefault("TORCH_CUDA_ARCH_LIST", "9.0")
    _EXTENSION = load(
        name=name,
        sources=[str(this_dir / "rr_mono_cuda_chunk_wave4_kernel.cu")],
        extra_cuda_cflags=[
            "-O3",
            "--use_fast_math",
            "-lineinfo",
...
def stage2_mono_wmma_lkq_dphi_chunk_owner_cuda(
    *,
    ...
    if not dout.is_cuda:
        raise RuntimeError("Wave 4 WMMA CUDA path requires CUDA tensors")
    ext = _load_extension()
