"""Minimal CUDA extension wrapper for the Wave 3 monolithic WMMA kernel."""
...
def _load_extension() -> Any:
    ...
    name = f"rr_mono_cuda_chunk_wave3_ext_t{threads}"
    if suffix:
        name = f"{name}_{suffix}"
    os.environ.setdefault("TORCH_CUDA_ARCH_LIST", "9.0")
    _EXTENSION = load(
        name=name,
        sources=[str(this_dir / "rr_mono_cuda_chunk_wave3_kernel.cu")],
        extra_cuda_cflags=[
            "-O3",
            "--use_fast_math",
            "-lineinfo",
...
def stage2_mono_wmma_lkq_dphi_chunk_owner_cuda(
    *,
    ...
    if not dout.is_cuda:
        raise RuntimeError("Wave 3 WMMA CUDA path requires CUDA tensors")
    ext = _load_extension()
