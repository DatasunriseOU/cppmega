...
#pragma unroll
    ...
    float* dssda_ptr = dssda_delta + seg_base + cs_i * kChunk + cs_j;
    if (p_panels == 1) {
      *dssda_ptr = acc;
    } else {
      atomicAdd(dssda_ptr, acc);
    }
  }
  __syncthreads();
  ...
  wmma_gemm_bf16_row_row_upper_k(lkq_masked_s, dphi_s, accum_s, 4, p_tiles, 4, kFusedChunk, kPPanel, kPPanel, true);
  __syncthreads();
...
void validate_inputs(
  ...
  TORCH_CHECK(chunk_size == kChunk, "Wave 4 WMMA path specializes chunk_size=16");
  ...
  TORCH_CHECK(R == kR, "Wave 4 WMMA path specializes R=4");
  TORCH_CHECK(N == 64, "Wave 4 WMMA path specializes N=64");
  TORCH_CHECK(P >= kPPanel && P % kPPanel == 0, "Wave 4 WMMA path requires P to be a positive multiple of 64");
...
py::dict stage2_mono_wmma_lkq_dphi_chunk_owner_metadata(const at::Tensor& dout) {
  ...
  TORCH_CHECK(dout.scalar_type() == at::kBFloat16, "Wave 4 WMMA metadata requires bf16");
  TORCH_CHECK(dout.dim() == 4, "dout must have shape [B, S, H, P]");
  ...
  out["tensor_core_gemms"] =
      "per P64 panel: LKQ=K@Q^T, dki=PsiV@dPhi^T, state=K@dstates[:,P64], triangular-pruned dPsi+=masked(LKQ)@dPhi[:,P64]";
  out["masked_lkq_smem"] = "reuses dead Q^T bf16 tile storage";
  out["dssda_accumulation"] = "single panel stores; multiple P64 panels atomicAdd partial DSSDA";
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def(
      ...
      &stage2_mono_wmma_lkq_dphi_chunk_owner,
      "Wave 4 P64-panel tensor-core WMMA state/LKQ/D chunk-owner kernel");
  m.def(
      "stage2_mono_wmma_lkq_dphi_chunk_owner_out",
      &stage2_mono_wmma_lkq_dphi_chunk_owner_out,
      "Wave 4 P64-panel tensor-core WMMA state/LKQ/D chunk-owner kernel into preallocated outputs");
  m.def(
      "stage2_mono_wmma_lkq_dphi_chunk_owner_metadata",
      &stage2_mono_wmma_lkq_dphi_chunk_owner_metadata,
      "Wave 4 P64-panel tensor-core WMMA state/LKQ/D chunk-owner kernel metadata");
}
