...
__global__ void stage2_mono_chunk_group_owner_kernel(
    ...
    float* __restrict__ dmimo_group_delta,
  ...
  __nv_bfloat16* q_t_or_masked_lkq_s = reinterpret_cast<__nv_bfloat16*>(cursor);
  ...
  float* dmimo_s = reinterpret_cast<float*>(cursor);
  ...
  const int chunk_group = static_cast<int>(pid % chunk_groups);
  const int h = static_cast<int>((pid / chunk_groups) % H);
  const int b = static_cast<int>(pid / (static_cast<int64_t>(chunk_groups) * H));
  if (b >= B) {
    return;
  }

  const int first_chunk = chunk_group * chunk_group_size;
  int last_chunk_exclusive = first_chunk + chunk_group_size;
  if (last_chunk_exclusive > nchunks) {
    last_chunk_exclusive = nchunks;
  }
  ...
  for (int chunk = last_chunk_exclusive - 1; chunk >= first_chunk; --chunk) {
    ...
    for (int idx = tid; idx < kFusedChunk * N; idx += blockDim.x) {
      ...
      q_t_or_masked_lkq_s[n * kFusedChunk + row] = float_to_bf16(q_val);
    }
    __syncthreads();

    wmma_gemm_bf16_row_row(k_s, q_t_or_masked_lkq_s, lkq_s, 4, 4, 4, N, kFusedChunk, kFusedChunk, false);
    __syncthreads();
    ...
    for (int idx = tid; idx < kFusedChunk * kFusedChunk; idx += blockDim.x) {
      ...
      q_t_or_masked_lkq_s[idx] = float_to_bf16(masked);
    }
...
#pragma unroll
  ...
  for (int idx = tid; idx < kR * P; idx += blockDim.x) {
    dmimo_group_delta[(((static_cast<int64_t>(b) * H + h) * chunk_groups + chunk_group) * kR * P) + idx] =
        dmimo_s[idx];
  }
}

__global__ void reduce_dmimo_groups_kernel(
    const float* __restrict__ dmimo_group_delta,
  ...
  for (int p = tid; p < P; p += blockDim.x) {
    float acc = 0.0f;
    for (int group = 0; group < chunk_groups; ++group) {
      acc += dmimo_group_delta[((((static_cast<int64_t>(b) * H + h) * chunk_groups + group) * R + r) * P) + p];
    }
...
size_t mono_chunk_group_owner_smem_bytes(int P) {
  ...
  const size_t float_bytes =
      (static_cast<size_t>(kFusedChunk) * kFusedChunk +
       static_cast<size_t>(kFusedChunk) * kPPanel +
       static_cast<size_t>(kChunk) * kChunk +
       static_cast<size_t>(kR) * P) *
      sizeof(float);
...
void validate_inputs(
  ...
  TORCH_CHECK(chunk_size == kChunk, "Wave 6 chunk-group owner specializes chunk_size=16");
  TORCH_CHECK(chunk_group_size > 0, "chunk_group_size must be positive");
  ...
  TORCH_CHECK(R == kR, "Wave 6 chunk-group owner specializes R=4");
  TORCH_CHECK(N == 64, "Wave 6 chunk-group owner specializes N=64");
  TORCH_CHECK(P >= kPPanel && P % kPPanel == 0, "Wave 6 chunk-group owner requires P to be a positive multiple of 64");
...
void stage2_mono_chunk_group_owner_out(
    const at::Tensor& dout,
    const at::Tensor& q_flat,
    const at::Tensor& k_flat,
    const at::Tensor& v,
    ...
    const at::Tensor& dmimo_group_delta,
  ...
  TORCH_CHECK(dmimo_group_delta.is_cuda() && dmimo_group_delta.is_contiguous(), "dmimo_group_delta must be contiguous CUDA");
  ...
  TORCH_CHECK(dmimo_group_delta.scalar_type() == at::kFloat, "dmimo_group_delta must be fp32");
  ...
  const int G = static_cast<int>(q_flat.size(2));
  const int nchunks = (S + chunk_size - 1) / chunk_size;
  const int chunk_groups = (nchunks + chunk_group_size - 1) / chunk_group_size;
  ...
  TORCH_CHECK(
      dmimo_group_delta.sizes() == at::IntArrayRef({B, H, chunk_groups, R, P}),
      "dmimo_group_delta shape mismatch");
  TORCH_CHECK(dssda_delta.sizes() == at::IntArrayRef({B, H, nchunks, chunk_size, chunk_size}), "dssda_delta shape mismatch");

  const int64_t total_programs = static_cast<int64_t>(B) * H * chunk_groups;
  const dim3 grid(static_cast<unsigned int>(total_programs));
  const dim3 block(kThreads);
  const size_t smem_bytes = mono_chunk_group_owner_smem_bytes(P);
  auto stream = at::cuda::getCurrentCUDAStream();

  C10_CUDA_CHECK(cudaFuncSetAttribute(
      stage2_mono_chunk_group_owner_kernel,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      static_cast<int>(smem_bytes)));

  stage2_mono_chunk_group_owner_kernel<<<grid, block, smem_bytes, stream>>>(
      ...
      dmimo_group_delta.data_ptr<float>(),
...
std::vector<at::Tensor> stage2_mono_chunk_group_owner(
    const at::Tensor& dout,
    const at::Tensor& q_flat,
    const at::Tensor& k_flat,
    const at::Tensor& v,
  ...
  at::Tensor dmimo_group_delta = at::empty({B, H, chunk_groups, R, P}, f32_opts);
  at::Tensor dssda_delta = at::empty({B, H, nchunks, chunk_size, chunk_size}, f32_opts);
  stage2_mono_chunk_group_owner_out(
      dout,
      q_flat,
      k_flat,
      v,
...
py::dict stage2_mono_chunk_group_owner_metadata(const at::Tensor& dout, int chunk_group_size) {
  TORCH_CHECK(dout.is_cuda(), "dout must be CUDA");
  TORCH_CHECK(dout.scalar_type() == at::kBFloat16, "Wave 6 scan metadata requires bf16");
  TORCH_CHECK(dout.dim() == 4, "dout must have shape [B, S, H, P]");
  TORCH_CHECK(chunk_group_size > 0, "chunk_group_size must be positive");

  cudaFuncAttributes attrs{};
  C10_CUDA_CHECK(cudaFuncGetAttributes(&attrs, stage2_mono_chunk_group_owner_kernel));
  ...
  C10_CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
      &active_blocks, stage2_mono_chunk_group_owner_kernel, kThreads, smem_bytes));
  ...
  out["owner"] = "B,H,chunk-group scan";
  out["scan_direction"] = "reverse chunks";
  out["chunk_group_size"] = chunk_group_size;
  out["chunk_groups"] = chunk_groups;
  out["chunk_group_owner_ctas"] = B * H * chunk_groups;
  out["chunk_group_owner_ctas_per_sm_at_132_sms"] =
      static_cast<double>(B) * H * chunk_groups / 132.0;
  out["chunks_per_cta"] = chunk_group_size;
  out["last_group_chunks"] = nchunks - (chunk_groups - 1) * chunk_group_size;
  out["logical_chunk_visits"] = B * H * nchunks;
  out["reduction_ctas"] = B * H * kR;
  out["outputs"] = "DV, per-group DMIMO_V partials reduced to final DMIMO_V, DSSDA";
  out["tensor_core_gemms"] =
      "per chunk: LKQ=K@Q^T once; per P64 panel: dki=PsiV@dPhi^T, state=K@dstates[:,P64], full-mask dPsi+=masked(LKQ)@dPhi[:,P64]";
  out["local_reuse"] = "LKQ/masked-LKQ reused across all P64 panels in one chunk; DMIMO_V accumulated in CTA-local shared memory per chunk group";
  out["dynamic_smem_bytes"] = static_cast<int64_t>(smem_bytes);
  out["dmimo_shared_bytes"] = static_cast<int64_t>(kR) * P * static_cast<int64_t>(sizeof(float));
  out["dmimo_group_scratch_bytes"] =
      static_cast<int64_t>(B) * H * chunk_groups * kR * P * static_cast<int64_t>(sizeof(float));
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def(
      "stage2_mono_chunk_group_owner",
      &stage2_mono_chunk_group_owner,
      "Wave 6 chunk-group owner WMMA state/LKQ/D kernel");
  m.def(
      "stage2_mono_chunk_group_owner_out",
      &stage2_mono_chunk_group_owner_out,
      "Wave 6 chunk-group owner WMMA state/LKQ/D kernel into preallocated outputs");
  m.def(
      "stage2_mono_chunk_group_owner_metadata",
      &stage2_mono_chunk_group_owner_metadata,
      "Wave 6 chunk-group owner WMMA state/LKQ/D kernel metadata");
}
