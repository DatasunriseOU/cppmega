"""Wave 7 row-stream CUDA prototype for Mamba3 bwd_bwd.

Wave 7 probes the opposite direction from Wave 5/6: keep chunk ownership, but
drop the full live LKQ and masked-LKQ tiles.  One CTA owns ``(B, H, chunk)``,
stages K/Q once, streams one LKQ row through shared memory, and keeps only one
P64 dPsi panel live.  The kernel emits DV, DSSDA, and per-chunk DMIMO_V
partials; a small reduction kernel produces final DMIMO_V.
"""
...
from rr_mono_cuda_chunk_wave7_extension import (  # noqa: E402
    stage2_mono_row_stream_chunk_owner_cuda,
    stage2_mono_row_stream_chunk_owner_cuda_metadata,
    stage2_mono_row_stream_chunk_owner_cuda_out,
)
...
def mono_row_stream_chunk_owner_cuda(
    inputs: dict[str, torch.Tensor],
    shape: Shape,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    return stage2_mono_row_stream_chunk_owner_cuda(
        dout=inputs["dout"],
...
def _cta_model(shape: Shape) -> dict[str, Any]:
    ...
    smem_model_bytes = {
        ...
        "dpsi_panel_shared": fused_chunk * P_PANEL * 4,
        "dssda_shared": shape.chunk * shape.chunk * 4,
        "lkq_row_shared": fused_chunk * 4,
    }
    return {
        "owner": "B,H,chunk row-stream",
        ...
        "live_intermediates": {
            ...
            "masked_lkq_tile_elements": 0,
            "streamed_lkq_row_elements": fused_chunk,
            "dpsi_panel_elements": fused_chunk * P_PANEL,
        },
        ...
        "tradeoff": "lower live set by recomputing LKQ rows and using scalar row consumers instead of WMMA LKQ reuse",
        "comparison_ms": {
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if shape.S % shape.chunk != 0:
        raise RuntimeError("Wave 7 harness currently expects S divisible by chunk")
    if shape.P % P_PANEL != 0:
        raise RuntimeError("Wave 7 row-stream owner requires P to be a multiple of 64")
    ...
    if device.type == "cuda":
        if dtype != torch.bfloat16:
            raise RuntimeError("Wave 7 row-stream owner CUDA path currently requires --dtype bf16")

        ref = _collapse_dmimo_chunk(mono_wmma_lkq_dphi_torch_reference(inputs, shape))
        got = mono_row_stream_chunk_owner_cuda(inputs, shape)
        torch.cuda.synchronize()
        correctness["wave7_row_stream_vs_bf16_staged_torch_reference"] = _diffs(ref, got)

        fp32_ref = _collapse_dmimo_chunk(mono_state_lkq_d_torch_reference(inputs, shape))
        correctness["wave7_row_stream_vs_wave1_fp32_reference"] = _diffs(fp32_ref, got)
        metadata["wave7_row_stream_chunk_owner"] = stage2_mono_row_stream_chunk_owner_cuda_metadata(
            inputs["dout"]
        )

        dv_out, dmimo_out, dmimo_chunk_out, dssda_out = _empty_row_stream_outputs(
            shape,
            dtype=dtype,
            device=device,
        )

        def run_row_stream() -> None:
            stage2_mono_row_stream_chunk_owner_cuda_out(
                dout=inputs["dout"],
                q_flat=inputs["q_flat"],
                k_flat=inputs["k_flat"],
        ...
        timings["wave7_row_stream_chunk_owner_slice"] = timer(
            run_row_stream,
            warmup=args.warmup,
            iters=args.iters,
        )
    ...
    row_ms = timings.get("wave7_row_stream_chunk_owner_slice", {}).get("mean_ms")
    if row_ms and args.shape == "productionish":
        tilelang = COMPARISON_CONTEXT["tilelang_stage2_bf1_bb0_bwd_bwd_productionish_ms"]
        timings["wave7_row_stream_chunk_owner_slice"]["ratio_vs_tilelang_bwd_bwd_prod"] = (
            row_ms / tilelang
        )
        timings["wave7_row_stream_chunk_owner_slice"]["speedup_vs_wave2_wmma_prod"] = (
            WAVE2_WMMA_PRODUCTIONISH_MS / row_ms
        )
        timings["wave7_row_stream_chunk_owner_slice"]["speedup_vs_wave5_scan_owner_prod"] = (
            WAVE5_SCAN_OWNER_PRODUCTIONISH_MS / row_ms
        )
        timings["wave7_row_stream_chunk_owner_slice"]["speedup_vs_wave6_chunk_group_owner_group8_prod"] = (
            WAVE6_CHUNK_GROUP_OWNER_PRODUCTIONISH_MS / row_ms
        )
        timings["wave7_row_stream_chunk_owner_slice"]["delta_ms_vs_wave5_scan_owner_prod"] = (
            row_ms - WAVE5_SCAN_OWNER_PRODUCTIONISH_MS
        )
        timings["wave7_row_stream_chunk_owner_slice"]["chunk_owner_ctas_per_sm_at_132_sms"] = (
            shape.B * shape.H * shape.nchunks / 132.0
        )
    ...
    return {
        ...
        "timings": timings,
        "read": [
            "Wave 7 is a low-live-set row-stream probe: one CTA owns one (B,H,chunk), but full raw LKQ and masked LKQ tiles are not resident.",
            "The CTA stages K/Q once, keeps one P64 dPhi/dPsi panel, streams a 64-element LKQ row, and accumulates DSSDA panel by panel.",
            "DV and DSSDA are emitted by the chunk-owner kernel; DMIMO_V uses per-chunk scratch plus a small final reduction kernel.",
            "This intentionally trades tensor-core reuse for lower shared-memory/register pressure to test whether more than one block/SM is reachable.",
        ],
    }
