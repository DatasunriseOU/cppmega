...
__global__ void stage2_mono_wmma_lkq_dphi_chunk_owner_kernel(
  ...
  wmma_gemm_bf16_row_row(k_s, q_t_s, lkq_s, 4, 4, 4, N, kFusedChunk, kFusedChunk, false);
  __syncthreads();
  ...
  wmma_gemm_bf16_row_col(psi_s, dphi_s, accum_s, 4, 4, p_tiles, P, P, kFusedChunk, false);
  __syncthreads();
...
#pragma unroll
  ...
  wmma_gemm_bf16_row_row(k_s, dstates + dstates_base, accum_s, 4, p_tiles, 4, N, P, P, false);
  __syncthreads();
  ...
  wmma_gemm_bf16_row_row_upper_k(lkq_masked_s, dphi_s, accum_s, 4, p_tiles, 4, kFusedChunk, P, P, true);
  __syncthreads();
...
size_t mono_wmma_lkq_dphi_smem_bytes(int P) {
  const size_t bf16_bytes =
      (static_cast<size_t>(kFusedChunk) * 64 +
       ...
       static_cast<size_t>(kFusedChunk) * P +
       static_cast<size_t>(kFusedChunk) * P) *
...
void validate_inputs(
  ...
  TORCH_CHECK(chunk_size == kChunk, "Wave 3 WMMA path specializes chunk_size=16");
  ...
  TORCH_CHECK(R == kR, "Wave 3 WMMA path specializes R=4");
  TORCH_CHECK(N == 64, "Wave 3 WMMA path specializes N=64");
  TORCH_CHECK(P % 16 == 0, "Wave 3 WMMA path requires P divisible by 16");
...
py::dict stage2_mono_wmma_lkq_dphi_chunk_owner_metadata(const at::Tensor& dout) {
  ...
  TORCH_CHECK(dout.scalar_type() == at::kBFloat16, "Wave 3 WMMA metadata requires bf16");
  TORCH_CHECK(dout.dim() == 4, "dout must have shape [B, S, H, P]");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def(
      ...
      &stage2_mono_wmma_lkq_dphi_chunk_owner,
      "Wave 3 tensor-core WMMA state/LKQ/D chunk-owner kernel");
  m.def(
      "stage2_mono_wmma_lkq_dphi_chunk_owner_out",
      &stage2_mono_wmma_lkq_dphi_chunk_owner_out,
      "Wave 3 tensor-core WMMA state/LKQ/D chunk-owner kernel into preallocated outputs");
  m.def(
      "stage2_mono_wmma_lkq_dphi_chunk_owner_metadata",
      &stage2_mono_wmma_lkq_dphi_chunk_owner_metadata,
      "Wave 3 tensor-core WMMA state/LKQ/D chunk-owner kernel metadata");
}
