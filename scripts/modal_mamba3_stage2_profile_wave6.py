"""Wave6 Modal profiling harness for Mamba3 stage2 bwd_bwd and related kernels.

This is intentionally separate from the wave5 timing harness. It reuses the
stage2 benchmark helper for steady CUDA-event timings/source metadata, then
tries focused Nsight Compute captures with LD_LIBRARY_PATH fixes. If NCU cannot
initialize, it falls back to Nsight Systems and the torch profiler artifacts
...
Example:
    ...
    GHCR_TAG=785c3fd CPPMEGA_MODAL_GPU=H200:2 timeout 3600s \
      modal run scripts/modal_mamba3_stage2_profile_wave6.py \
        --run-id mamba3_stage2_profile_wave6_h200_prod_20260429_1 \
        --shape-csv productionish \
        --variant-csv baseline,stage2_force_nontma \
        --phase-csv bwd_bwd,bwd_fwd \
        --warmup 2 \
        --iters 8
"""
...
APP_NAME = "cppmega-mamba3-stage2-profile-wave6"
SOURCE_ROOT = "/opt/state-spaces-mamba"
CPPMEGA_ROOT = "/opt/cppmega"
BENCH_ROOT = "/benchmarks"
BENCH_PREFIX = "mamba3_stage2_profile_wave6"
DEFAULT_NCU_BIN = os.environ.get("CPPMEGA_NCU_BIN", "/usr/local/cuda/bin/ncu")
...
def _environment_report(run_dir: str) -> dict[str, Any]:
    ...
    ncu_bin = os.environ.get("CPPMEGA_NCU_BIN", DEFAULT_NCU_BIN)
    ...
    return [
        ...
        },
        {
            ...
            + [
                ...
                "--export",
                f"{report_base}_occupancy_memory",
            ],
        },
        {
            "name": "basic_ldfix",
...
def _run_ncu_profile(
    *,
    ...
    ncu_bin = os.environ.get("CPPMEGA_NCU_BIN", DEFAULT_NCU_BIN)
    if not os.path.exists(ncu_bin) and shutil.which("ncu"):
        ncu_bin = shutil.which("ncu") or ncu_bin
    if not os.path.exists(ncu_bin):
        return {"status": "unavailable", "reason": f"ncu not found: {ncu_bin}"}
    ...
    for attempt in _ncu_attempts(ncu_bin, report_base, launches):
        ...
        env = ld_env if attempt["ld_fix"] else base_env
...
def run_profile(
    ...
    selected_variants = stage2._selected_variants(variant_csv)
