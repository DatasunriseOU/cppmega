"""Modal H200 runner for the Wave1 monolithic Triton reuse model."""
...
APP_NAME = "cppmega-mamba3-mono-triton-reuse-wave1"
...
def run_remote(
    ...
    try:
        ...
        path = bench_dir / "rr_diag_wave1_mono_triton_reuse_model.py"
        print(f"loading {path}", flush=True)
        spec = importlib.util.spec_from_file_location("rr_diag_wave1_mono_triton_reuse_model", path)
        if spec is None or spec.loader is None:
            raise RuntimeError(f"could not load {path}")
        mod = importlib.util.module_from_spec(spec)
        sys.modules["rr_diag_wave1_mono_triton_reuse_model"] = mod
