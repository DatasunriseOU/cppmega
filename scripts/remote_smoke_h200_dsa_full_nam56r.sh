#!/usr/bin/env bash
# Full NAM56R + DSA on 8xH200 with real data.
#
# Unlike remote_smoke_h200_dsa.sh which used a toy model (hidden=256),
# this script runs the full 4.73B NAM56R architecture:
#   - hidden_size=3584, ffn_hidden_size=18944, num_attention_heads=28
#   - 52 layers with AEMEAEMEAEMR hybrid pattern
#   - Branch B (noconv) spec: vanilla Mamba-2 SSD + Mamba3 B/C features
#   - DSA (experimental_attention_variant=dsa) on attention layers
#   - MTP, MoE, MLA enabled
#   - Real data: clang_semantic_4k_v10_train
#
# Usage:
#   REMOTE_HOST=h200_1 \
#   REMOTE_ZONE=LOCATION_1 \
#   bash scripts/remote_smoke_h200_dsa_full_nam56r.sh
#
# Or with SSH directly to bench3:
#   bash scripts/remote_smoke_h200_dsa_full_nam56r.sh
set -euo pipefail
...
CPPMEGA_RUN_ID="${CPPMEGA_RUN_ID:-cppmega_nam56r_dsa_full_noconv}"
REMOTE_VENV="${REMOTE_VENV:-${REMOTE_ROOT}/venv}"
REMOTE_LOG="${REMOTE_LOG:-${REMOTE_ROOT}/cppmega-root/cppmega/${CPPMEGA_RUN_ID}.log}"
REMOTE_TMP_SCRIPT="${REMOTE_TMP_SCRIPT:-/tmp/cppmega-remote-dsa-full-nam56r.sh}"
LOCAL_TMP_SCRIPT="$(mktemp -t cppmega-remote-dsa-full.XXXXXX.sh)"
...
WORKDIR="$(mktemp -d /tmp/cppmega-dsa-full.XXXXXX)"
trap 'rm -rf "${WORKDIR}"' EXIT

cp "${REMOTE_ROOT}/cppmega-root/megatron-lm/pretrain_mamba.py" "${WORKDIR}/pretrain_mamba.py"
...
# Apply DSA Megatron patches (idempotent)
python - <<'PY'
