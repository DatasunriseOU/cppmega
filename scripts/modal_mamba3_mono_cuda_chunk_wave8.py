"""Modal runner for the Wave 8 tile-stream WMMA chunk-owner CUDA prototype."""
...
APP_NAME = "cppmega-mamba3-mono-cuda-chunk-wave8-" + re.sub(r"[^0-9A-Za-z]+", "-", GPU_SPEC).lower()
...
def run_remote(
    ...
    os.environ["RR_DIAG_THREADS"] = str(threads)
    os.environ["RR_DIAG_CUDA_EXT_SUFFIX"] = f"mono_wave8_tile_stream_dki_wmma_t{threads}"
    os.environ["RR_DIAG_CUDA_VERBOSE_BUILD"] = "1" if verbose_build else "0"
    os.environ["TORCH_CUDA_ARCH_LIST"] = "9.0"

    for module in (
        "rr_mono_cuda_chunk_wave1",
        "rr_mono_cuda_chunk_wave2",
        "rr_mono_cuda_chunk_wave8",
        "rr_mono_cuda_chunk_wave8_extension",
    ):
        sys.modules.pop(module, None)

    bench_dir = pathlib.Path(CPPMEGA_ROOT) / "upstream_prs/examples/13_tilelang_floormod_dbz"
    sys.path.insert(0, str(bench_dir))
    path = bench_dir / "rr_mono_cuda_chunk_wave8.py"
    module_name = "rr_mono_cuda_chunk_wave8_" + re.sub(r"[^0-9A-Za-z_]+", "_", shape_csv)
...
def main(
    ...
    print(
        ...
        f"threads={threads} verbose_build={verbose_build}"
    )
    if background:
        call = run_remote.spawn(shape_csv, iters, warmup, threads, verbose_build)
        print(f"FUNCTION_CALL_ID={call.object_id}")
        return
    results = run_remote.remote(shape_csv, iters, warmup, threads, verbose_build)
    for result in results:
        print(json.dumps(result, indent=2, sort_keys=True))
    print("SUMMARY_JSON=" + json.dumps(results, sort_keys=True))
