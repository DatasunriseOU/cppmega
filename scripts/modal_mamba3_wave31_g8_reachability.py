"""Modal H200:2 Mamba3 G=8 backward reachability harness.

Wave31 extends the Wave30 full-boundary attention-debug harness with an
env-gated source patch for Mamba3 MIMO backward grouped-head support. This specifically
targets the NAM56R blocker:

    ValueError: G value of 8 is not currently supported!

Fallback cases are for debug/profiling reachability only and are not
...
APP_NAME = "cppmega-wave31-g8-reachability"
RESULTS_VOL = "cppmega-mamba3-benchmarks"
BENCH_DIR = "/benchmarks/mamba3_wave31_g8_reachability"
...
_CASES: dict[str, dict[str, Any]] = {
    ...
    "fallback_auto_full": {
        "description": "Fallback reachability path: TE attention_backend=auto at full shape.",
...
try:
    ...
    print(f"[wave31_shim] static_context alias skipped: {_exc}", file=sys.stderr)
...
if os.environ.get("CPPMEGA_MAMBA3_MIMO", "0") == "1":
    try:
        ...
        print("[wave31_shim] MIMO config hook installed")
    except Exception as _exc:
        print(f"[wave31_shim] MIMO config hook failed: {_exc}", file=sys.stderr)
...
try:
    ...
    if not getattr(_Mamba3, "_cppmega_fp32_bias_hook", False):
        ...
        print("[wave31_shim] Mamba3 fp32-bias hook installed")
except Exception as _exc:
    print(f"[wave31_shim] Mamba3 fp32-bias hook failed: {_exc}", file=sys.stderr)
...
print(f"[wave31_shim] DSA indexer dtype={os.environ.get('CPPMEGA_DSA_INDEXER_DTYPE', 'bf16')}")
print(f"[wave31_shim] DSA sparse mode={os.environ.get('CPPMEGA_DSA_SPARSE_MODE', 'tilelang')}")
print(f"[wave31_shim] DSA KL mode={os.environ.get('CPPMEGA_DSA_KL_MODE', 'head_streaming')}")
...
def _build_train_cmd(
    ...
    vpp_size = 2
    ep_size = 1
    mbs = int(os.environ.get("CPPMEGA_WAVE31_MBS", os.environ.get("CPPMEGA_WAVE30_MBS", "1")))
    gbs = int(os.environ.get("CPPMEGA_WAVE31_GBS", os.environ.get("CPPMEGA_WAVE30_GBS", "8")))
    seq_len = int(os.environ.get("CPPMEGA_WAVE31_SEQ_LEN", os.environ.get("CPPMEGA_WAVE30_SEQ_LEN", str(case["seq_len"]))))
    mtp_depths = int(os.environ.get("CPPMEGA_WAVE31_MTP_DEPTHS", os.environ.get("CPPMEGA_WAVE30_MTP_DEPTHS", "2")))
    dsa_loss_coeff = float(
        os.environ.get("CPPMEGA_WAVE31_DSA_LOSS_COEFF", os.environ.get("CPPMEGA_WAVE30_DSA_LOSS_COEFF", "0.001"))
    )
    attention_backend = os.environ.get("CPPMEGA_WAVE31_ATTN_BACKEND", os.environ.get("CPPMEGA_WAVE30_ATTN_BACKEND", str(case["attention_backend"])))
...
def gate(
    ...
    cmd, meta = _build_train_cmd(workdir, dataset, train_iters, profile=profile, case_label=case_label)
    ...
    result["stage2_applier_pre_rollback"] = _applier("rollback", env)
...
def debug_sweep(
    ...
    try:
        for case_label in selected:
            result["stage2_applier_case_rollback_before_" + case_label] = _applier("rollback", env)
            cmd, meta = _build_train_cmd(workdir, dataset, train_iters, profile=True, case_label=case_label)
            (out_dir / f"command_{case_label}.json").write_text(
            ...
            for variant in variants:
                ...
                run = _run_streaming(cmd, env, log_file, timeout_per_case_s, cwd=workdir)
                metrics = _parse_log(log_file, meta["tokens_per_iter"])
                row = {
                    "case_label": case_label,
                    "variant": variant,
