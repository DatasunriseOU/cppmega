...
env-gated source patch for Mamba3 MIMO backward GQA support. This specifically
...
print(json.dumps({
    "path": str(path),
    "gqa_bwd": "elif H % G == 0:" in text,
    "gqa_bwd_error_guard": "G must divide H" in text,
    "gqa_bwd_dq_reduce": "dq_tilelang.view(B, S, R, G, hpg, N).sum(dim=4)" in text,
...
def preflight(run_id: str = "") -> dict[str, Any]:
    ...
    result = {
        ...
        "gqa_applier_pre_rollback": gqa_pre_rollback,
...
def gate(
    ...
    try:
        for variant in ("gqa_bwd_baseline", "gqa_bwd_stage2_force_nontma_bf1_bb0"):
            if variant == "gqa_bwd_stage2_force_nontma_bf1_bb0":
...
def debug_sweep(
    ...
    try:
        for case_label in selected:
            ...
            variants = ["gqa_bwd_baseline"]
            if include_stage2:
                variants.append("gqa_bwd_stage2_force_nontma_bf1_bb0")
            for variant in variants:
                if variant == "gqa_bwd_stage2_force_nontma_bf1_bb0":
                    ...
                    if previous.get("variant") != "gqa_bwd_baseline" or not baseline_reached_target:
                        break
