...
cppmega_deprecated_script_guard "$(basename "$0")" \
  "scripts/remote_smoke_h200_dsa_9_4_m.sh or scripts/local_gb10_quarter_train.sh"
...
python -m torch.distributed.run --nproc_per_node=8 "${REMOTE_WORKDIR}/pretrain_mamba.py" \
  ...
  --use-flash-attn \
  --attention-backend "${CPPMEGA_ATTN_BACKEND:-flash}" \
