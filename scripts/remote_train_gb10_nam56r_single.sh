...
export PYTHONPATH="${REMOTE_ROOT}/cppmega:${REMOTE_ROOT}/megatron-lm:/home/dave/megatron-lm:${PYTHONPATH:-}"
...
print("SYS ARGV IS:", sys.argv, flush=True)
...
# Apply cppmega structure metadata patch at the very beginning if enabled.
if os.environ.get("CPPMEGA_STRUCTURE_ENABLED", "0") == "1":
    try:
        import cppmega.megatron.structure_dataset_patch
    except Exception as exc:
        print(f"[gb10-wrapper] failed to import structure patch: {exc}", flush=True)
