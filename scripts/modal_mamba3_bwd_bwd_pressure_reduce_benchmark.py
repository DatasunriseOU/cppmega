"""Bounded Modal benchmark for cheap Mamba3 MIMO bwd_bwd pressure reductions.

Compares non-production variants on Hopper GPUs:
  ...
  * pressure_* variants: stage2_force_nontma plus one small bwd_bwd-only
    source-level live-set/copy-pressure reduction.
...
APP_NAME = "cppmega-mamba3-bwd-bwd-pressure-reduce-benchmark"
SOURCE_ROOT = "/opt/state-spaces-mamba"
CPPMEGA_ROOT = "/opt/cppmega"
BENCH_ROOT = "/benchmarks"
BENCH_PREFIX = "mamba3_bwd_bwd_pressure_reduce_benchmark"
...
VARIANTS: dict[str, dict[str, Any]] = {
    ...
    "pressure_direct_bias": {
        "patch": "stage2_force_nontma",
        "patch_file": "mamba3_bwd_stage2_force_nontma.patch",
        "extra_patch_files": ["mamba3_bwd_bwd_pressure_reduce_direct_bias.patch"],
        "flattened_inputs": True,
        "flat_qk_dot": True,
        "bf_threads": 128,
        "bf_num_stages": 1,
    ...
    "pressure_rotary_onetmp": {
        "patch": "stage2_force_nontma",
        "patch_file": "mamba3_bwd_stage2_force_nontma.patch",
        "extra_patch_files": ["mamba3_bwd_bwd_pressure_reduce_rotary_onetmp.patch"],
        "flattened_inputs": True,
        "flat_qk_dot": True,
        "bf_threads": 128,
        "bf_num_stages": 1,
    ...
    "pressure_dgamma_serial": {
        "patch": "stage2_force_nontma",
        "patch_file": "mamba3_bwd_stage2_force_nontma.patch",
        "extra_patch_files": ["mamba3_bwd_bwd_pressure_reduce_dgamma_serial.patch"],
        "flattened_inputs": True,
        "flat_qk_dot": True,
        "bf_threads": 128,
        "bf_num_stages": 1,
    ...
    "pressure_direct_bias_rotary_onetmp": {
        "patch": "stage2_force_nontma",
        "patch_file": "mamba3_bwd_stage2_force_nontma.patch",
        "extra_patch_files": [
...
def _apply_extra_patches(dst: str, patch_names: list[str]) -> dict[str, Any]:
    results = []
    for patch_name in patch_names:
        result = _apply_patch(dst, patch_name)
        results.append(result)
        if result["patch_rc"] != 0:
            break
    return {
        "extra_patch_files": patch_names,
        "extra_patch_results": results,
        "extra_patch_rc": results[-1]["patch_rc"] if results else 0,
    }
...
def _prepare_variant(variant: str) -> tuple[str, dict[str, Any]]:
    ...
    if patch_kind == "stage2_force_nontma":
        ...
        if patch_meta["patch_rc"] != 0:
            return dst, meta
        extra_patch_files = list(VARIANTS[variant].get("extra_patch_files", []))
        if extra_patch_files:
            extra_meta = _apply_extra_patches(dst, extra_patch_files)
            meta.update(extra_meta)
            if extra_meta["extra_patch_rc"] != 0:
                meta["patch_rc"] = extra_meta["extra_patch_rc"]
                return dst, meta
...
def main(
    ...
    variant_csv: str = "baseline,stage2_force_nontma,pressure_direct_bias,pressure_rotary_onetmp,pressure_dgamma_serial",
    warmup: int = 2,
    iters: int = 8,
