"""Modal H200 microbench for Mamba3 bwd_bwd accumulator-surface reduction wave 3.
...
APP_NAME = "cppmega-mamba3-bwd-bwd-surface-reduction-wave3"
BLOCK_J = 16
        ...
        for tok in tl.static_range(0, 16):
            ...
            for dst in tl.static_range(0, 4):
                ...
                for src in tl.static_range(0, 4):
                    ...
                    coeff_vec = tl.sum(tl.where(offs_r[None, :] == src, tl.trans(g_for_dot), 0.0), axis=1)
                    coeff = tl.sum(tl.where(offs_r == dst, coeff_vec, 0.0), axis=0)
                    dk += tl.where(offs_f[:, None] == tok * R + dst, coeff * q_src[None, :], 0.0)
        ...
        for tok in tl.static_range(0, 16):
            ...
            for dst in tl.static_range(0, 4):
                for src in tl.static_range(0, 4):
                    ...
                    coeff_vec = tl.sum(tl.where(offs_r[None, :] == src, g_for_dot, 0.0), axis=1)
                    coeff = tl.sum(tl.where(offs_r == dst, coeff_vec, 0.0), axis=0)
                    dq += tl.where(offs_f[:, None] == tok * R + dst, coeff * k_src[None, :], 0.0)
...
def _run_shape(shape: Shape, warmup: int, iters: int) -> dict[str, Any]:
    ...
    if shape.P != 128 or shape.fused_chunk != 64 or shape.N != 64 or shape.R != 4:
        result.update({"status": "skipped", "reason": "wave3 prototype specializes P=128,F=64,N=64,R=4"})
        return result
...
def run_probe(requested_gpu: str, shape_csv: str, warmup: int, iters: int) -> dict[str, Any]:
    ...
    try:
        ...
        for shape in _selected_shapes(shape_csv):
            print(f"[surface-wave3] shape={shape.name} block_j={BLOCK_J}", flush=True)
