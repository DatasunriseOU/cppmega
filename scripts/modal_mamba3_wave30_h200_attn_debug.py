"""Modal H200:2 attention-debug and Mamba3 backward reachability harness.

Wave30 extends the Wave29 full-boundary gate with explicit Transformer Engine
attention debug logging and labeled fallback cases. Fallback cases are for
debug/profiling reachability only and are not production-throughput claims.
...
APP_NAME = "cppmega-wave30-modal-h200-attn-debug"
RESULTS_VOL = "cppmega-mamba3-benchmarks"
BENCH_DIR = "/benchmarks/mamba3_wave30_modal_h200_attn_debug"
...
try:
    ...
    print(f"[wave30_shim] static_context alias skipped: {_exc}", file=sys.stderr)
...
if os.environ.get("CPPMEGA_MAMBA3_MIMO", "0") == "1":
    try:
        ...
        print("[wave30_shim] MIMO config hook installed")
    except Exception as _exc:
        print(f"[wave30_shim] MIMO config hook failed: {_exc}", file=sys.stderr)
...
try:
    ...
    if not getattr(_Mamba3, "_cppmega_fp32_bias_hook", False):
        ...
        print("[wave30_shim] Mamba3 fp32-bias hook installed")
except Exception as _exc:
    print(f"[wave30_shim] Mamba3 fp32-bias hook failed: {_exc}", file=sys.stderr)
...
print(f"[wave30_shim] DSA indexer dtype={os.environ.get('CPPMEGA_DSA_INDEXER_DTYPE', 'bf16')}")
print(f"[wave30_shim] DSA sparse mode={os.environ.get('CPPMEGA_DSA_SPARSE_MODE', 'tilelang')}")
print(f"[wave30_shim] DSA KL mode={os.environ.get('CPPMEGA_DSA_KL_MODE', 'head_streaming')}")
...
def _build_train_cmd(
    ...
    vpp_size = 2
    ep_size = 1
    mbs = int(os.environ.get("CPPMEGA_WAVE30_MBS", os.environ.get("CPPMEGA_WAVE29_MBS", "1")))
    gbs = int(os.environ.get("CPPMEGA_WAVE30_GBS", os.environ.get("CPPMEGA_WAVE29_GBS", "8")))
    seq_len = int(os.environ.get("CPPMEGA_WAVE30_SEQ_LEN", str(case["seq_len"])))
    mtp_depths = int(os.environ.get("CPPMEGA_WAVE30_MTP_DEPTHS", os.environ.get("CPPMEGA_WAVE29_MTP_DEPTHS", "2")))
    dsa_loss_coeff = float(
        os.environ.get("CPPMEGA_WAVE30_DSA_LOSS_COEFF", os.environ.get("CPPMEGA_WAVE29_DSA_LOSS_COEFF", "0.001"))
    )
    attention_backend = os.environ.get("CPPMEGA_WAVE30_ATTN_BACKEND", str(case["attention_backend"]))
    use_flash_attn = os.environ.get(
        "CPPMEGA_WAVE30_USE_FLASH_ATTN",
        "1" if case["use_flash_attn"] else "0",
    ).strip().lower() not in {"0", "false", "no", "off"}
    cuda_graph = os.environ.get(
        "CPPMEGA_WAVE30_CUDA_GRAPH",
        "1" if case["cuda_graph"] else "0",
    ).strip().lower() not in {"0", "false", "no", "off"}
...
def debug_sweep(
    ...
    try:
        for case_label in selected:
            result["applier_case_rollback_before_" + case_label] = _applier("rollback", env)
            ...
            for variant in ("baseline", "stage2_force_nontma_bf1_bb0"):
                if variant == "stage2_force_nontma_bf1_bb0":
                    ...
                    if previous.get("variant") != "baseline" or not baseline_reached_target:
                        break
                    result["applier_apply_" + case_label] = _applier("apply", env)
                ...
                results_vol.commit()
                if variant == "stage2_force_nontma_bf1_bb0":
                    result["applier_rollback_after_" + case_label] = _applier("rollback", env)
...
def launch_debug_sweep(
    ...
    result = debug_sweep.remote(run_id or f"debug_sweep_{_utc_stamp()}", train_iters, cases, timeout_per_case_s)
    print(json.dumps(result, indent=2, sort_keys=True))
