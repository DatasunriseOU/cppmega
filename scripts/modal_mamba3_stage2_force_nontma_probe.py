"""Modal Hopper probe for Mamba3 bwd stage-2 force non-TMA slices.

This is a temp-only harness. It copies upstream ``mamba3_mimo_bwd.py`` into a
container tempdir, applies the non-production stage-2 patch, and checks whether
``num_stages=2`` smokes without the rank-3 float32 TMA descriptor init failure.

Run examples:

    CPPMEGA_MODAL_GPU=H100:2 modal run scripts/modal_mamba3_stage2_force_nontma_probe.py
    CPPMEGA_MODAL_GPU=H200:2 modal run scripts/modal_mamba3_stage2_force_nontma_probe.py
"""
...
APP_NAME = "cppmega-mamba3-stage2-force-nontma-probe"
SOURCE_ROOT = "/opt/state-spaces-mamba"
CPPMEGA_ROOT = "/opt/cppmega"
PATCH_BASENAME = "mamba3_bwd_stage2_force_nontma.patch"
...
def _patched_source_report(path: str) -> dict[str, Any]:
    ...
    large_q_copy = "T.copy(Q[i_b, fused_chunk_start:fused_chunk_start+fused_chunk_size"
    large_k_copy = "T.copy(K[i_b, fused_chunk_start:fused_chunk_start+fused_chunk_size"
    qk_copy = "T.copy(QK_DOT[i_b, i_h, chunk_start:chunk_start+chunk_size, :], qk_dot_shared)"
    return {
        "patch": PATCH_BASENAME,
        "disable_tma_count": text.count("disable_tma=True"),
        "deprecated_global_tma_disable_count": text.count("TL_DISABLE_TMA_LOWER"),
        "large_q_copy_count": text.count(large_q_copy),
        "large_k_copy_count": text.count(large_k_copy),
        "qk_tma_capable_copy_count": text.count(qk_copy),
        "qk_shared_direct_refs": text.count("qk_dot_shared[cs, r_out * R + r_in]"),
        "remaining_percent_R": text.count("% R"),
        "remaining_floor_div_R": text.count("// R"),
        "large_q_copy_disable_tma": "disable_tma=True" in _snippet(text, large_q_copy, radius=200)
        if large_q_copy in text
        else None,
        "large_k_copy_disable_tma": "disable_tma=True" in _snippet(text, large_k_copy, radius=200)
        if large_k_copy in text
        else None,
        "qk_copy_disable_tma": "disable_tma=True" in _snippet(text, qk_copy, radius=200)
        if qk_copy in text
        else None,
    }
...
def _prepare_module() -> tuple[str, dict[str, Any]]:
    ...
    work = tempfile.mkdtemp(prefix="cppmega_mamba3_stage2_force_nontma_")
    ...
    if proc.returncode == 0:
        meta["patched_source"] = _patched_source_report(dst)
    return dst, meta
...
def _stage_matrix() -> list[tuple[int, int]]:
    raw = os.environ.get("CPPMEGA_MAMBA3_STAGE_MATRIX", "2,0;0,2;2,2;1,1")
    matrix: list[tuple[int, int]] = []
    for item in raw.split(";"):
        item = item.strip()
        if not item:
            continue
        bf_raw, bb_raw = item.split(",", maxsplit=1)
        matrix.append((int(bf_raw), int(bb_raw)))
    return matrix


@app.function(image=_image(), gpu=GPU_SPEC, timeout=600)
def run_probe(requested_gpu: str) -> dict[str, Any]:
    stage_matrix = _stage_matrix()
