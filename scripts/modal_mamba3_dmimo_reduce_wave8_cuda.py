"""Modal H200 runner for the wave8 DMIMO_V reduction CUDA prototypes."""
...
APP_NAME = "cppmega-mamba3-dmimo-reduce-wave8-cuda"
...
def run_remote(
    ...
    path = bench_dir / "rr_diag_wave8_dmimo_reduce_cuda.py"
    spec = importlib.util.spec_from_file_location("rr_diag_wave8_dmimo_reduce_cuda", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"could not load {path}")
    mod = importlib.util.module_from_spec(spec)
    sys.modules["rr_diag_wave8_dmimo_reduce_cuda"] = mod
