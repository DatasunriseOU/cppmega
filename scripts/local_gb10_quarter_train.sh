...
export CPPMEGA_MUON_FP32_MATMUL_PREC="${CPPMEGA_MUON_FP32_MATMUL_PREC:-medium}"
...
echo "[local-quarter] optimizer=${CPPMEGA_OPTIMIZER} param_storage=${CPPMEGA_PARAM_STORAGE} muon_scalar=${CPPMEGA_MUON_SCALAR_OPTIMIZER} muon_ns_steps=${CPPMEGA_MUON_NUM_NS_STEPS} muon_fp32_matmul_prec=${CPPMEGA_MUON_FP32_MATMUL_PREC} muon_dtype_audit=${CPPMEGA_MUON_DTYPE_AUDIT}"
...
case "${CPPMEGA_OPTIMIZER}" in
  muon|dist_muon|adaptive_muon)
    OPTIMIZER_ARGS+=(
      ...
      --muon-fp32-matmul-prec "${CPPMEGA_MUON_FP32_MATMUL_PREC}"
