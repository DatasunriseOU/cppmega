"""Modal runner for the Wave 7 row-stream chunk-owner CUDA prototype."""
...
APP_NAME = "cppmega-mamba3-mono-cuda-chunk-wave7-" + re.sub(r"[^0-9A-Za-z]+", "-", GPU_SPEC).lower()
...
def run_remote(
    ...
    os.environ["RR_DIAG_CUDA_EXT_SUFFIX"] = f"mono_wave7_row_stream_t{threads}"
    ...
    for module in (
        ...
        "rr_mono_cuda_chunk_wave7",
        "rr_mono_cuda_chunk_wave7_extension",
    ):
        sys.modules.pop(module, None)

    bench_dir = pathlib.Path(CPPMEGA_ROOT) / "upstream_prs/examples/13_tilelang_floormod_dbz"
    sys.path.insert(0, str(bench_dir))
    path = bench_dir / "rr_mono_cuda_chunk_wave7.py"
    module_name = "rr_mono_cuda_chunk_wave7_" + re.sub(r"[^0-9A-Za-z_]+", "_", shape_csv)
