"""Modal H200 runner for the wave9 state/LKQ/D CUDA ownership probe."""
...
APP_NAME = "cppmega-mamba3-state-lkq-d-wave9-cuda"
...
def run_remote(
    ...
    try:
        ...
        path = bench_dir / "rr_diag_wave9_state_lkq_d_cuda.py"
        print(f"loading {path}", flush=True)
        spec = importlib.util.spec_from_file_location("rr_diag_wave9_state_lkq_d_cuda", path)
        if spec is None or spec.loader is None:
            raise RuntimeError(f"could not load {path}")
        mod = importlib.util.module_from_spec(spec)
        sys.modules["rr_diag_wave9_state_lkq_d_cuda"] = mod
        spec.loader.exec_module(mod)
        print("module loaded; starting run", flush=True)
...
def main(
    ...
    skip_reference_timing: bool = False,
) -> None:
