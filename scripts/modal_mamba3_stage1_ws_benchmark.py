"""Bounded Modal benchmark for Mamba3 MIMO stage-1 WS qk_shared_direct.

Compares three non-production variants on Hopper GPUs:
  * baseline: upstream non-TMA/non-WS TileLang kernels
  * qk_direct_non_ws: old layout-fix qk_direct path with stages=0
  * qk_shared_direct_stage1_ws: qk_shared_direct with bf/bb num_stages=1
...
Run examples:
    ...
    GHCR_TAG=785c3fd CPPMEGA_MODAL_GPU=H200:2 timeout 10m \
        modal run scripts/modal_mamba3_stage1_ws_benchmark.py \
        --shape-csv representative --iters 8 --warmup 2

    GHCR_TAG=785c3fd CPPMEGA_MODAL_GPU=H100:2 timeout 10m \
        modal run scripts/modal_mamba3_stage1_ws_benchmark.py \
        --shape-csv smoke --iters 6 --warmup 2
"""
...
APP_NAME = "cppmega-mamba3-stage1-ws-benchmark"
SOURCE_ROOT = "/opt/state-spaces-mamba"
CPPMEGA_ROOT = "/opt/cppmega"
BENCH_ROOT = "/benchmarks"
BENCH_PREFIX = "mamba3_stage1_ws_benchmark"
...
VARIANTS: dict[str, dict[str, Any]] = {
    ...
    "qk_shared_direct_stage1_ws": {
        "patch": "layout_fix_plus_qk_shared_direct",
        "flattened_inputs": True,
        "flat_qk_dot": True,
        "bf_threads": 128,
        "bf_num_stages": 1,
...
def _prepare_variant(variant: str) -> tuple[str, dict[str, Any]]:
    ...
    work = tempfile.mkdtemp(prefix=f"cppmega_mamba3_stage1_ws_{variant}_")
...
def _import_variant(path: str, variant: str, shape: Shape):
    ...
    name = f"cppmega_mamba3_stage1_ws_{shape.name}_{variant}_{int(time.time() * 1000)}"
