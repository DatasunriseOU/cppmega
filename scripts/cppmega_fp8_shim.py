    ...
    try:
        ...
        def _cppmega_set_mxfp8_sidecar_refs_untracked(_x, _sidecar):
            if _te_mxfp8_linear_kernel_contract == "compact_direct_v1":
                _cppmega_record_bwd_stat(
                    "mxfp8_tn_adapter_te_emit_failed",
                    "compact_direct_v1_forbids_untracked_sidecar_attach",
                )
                raise RuntimeError(
                    "compact_direct_v1 forbids untracked MXFP8 rowwise-transpose sidecars"
                )
            _old_sidecar = getattr(_x, _cppmega_mxfp8_tn_sidecar_attr, None)
            if _old_sidecar is None:
                _old_sidecar = getattr(_x, "_te_rowwise_transpose_for_backward", None)
        ...
        def _cppmega_register_mxfp8_sidecar(_x, _sidecar, *, persistent=False):
            if _te_mxfp8_linear_kernel_contract == "compact_direct_v1":
                _cppmega_record_bwd_stat(
                    "mxfp8_tn_adapter_te_emit_failed",
                    "compact_direct_v1_forbids_sidecar_registry",
                )
                raise RuntimeError(
                    "compact_direct_v1 forbids MXFP8 transpose sidecar registry use"
                )
            persistent = False
        ...
        def _cppmega_mxfp8_colwise_as_rowwise_transpose(_x, *, _ignore_saved=False):
            ...
            if not _ignore_saved and (
                ...
                if _te_mxfp8_linear_kernel_contract == "compact_direct_v1":
                    _cppmega_record_bwd_stat("mxfp8_tn_adapter_saved_transpose_rejected")
                    _cppmega_record_bwd_stat(
                        "mxfp8_tn_adapter_missing_sidecar_strict",
                        "compact_direct_v1_forbids_saved_transpose_operand",
                    )
                    raise ValueError(
                        "compact_direct_v1 forbids saved rowwise-transpose MXFP8 "
                        f"operands; {_cppmega_mxfp8_debug_desc(_x)}"
                    )
            ...
            if _te_mxfp8_linear_kernel_contract == "compact_direct_v1":
                ...
                raise ValueError(
                    "compact_direct_v1 forbids copy-based MXFP8 transpose "
                    f"materialization for {_cppmega_mxfp8_debug_desc(_x)}"
                )
            warnings.warn(
