...
if (
    ...
    try:
        ...
        _cppmega_te_bwd_stats = {
            ...
            "mxfp8_tn_adapter_te_autograd_saved_operand": 0,
            "mxfp8_tn_adapter_fused_producer_operand": 0,
            "mxfp8_tn_adapter_registry_sidecar_operand": 0,
        ...
        def _cppmega_record_gemm_ready_saved_operand(_x):
            _cppmega_record_bwd_stat("mxfp8_tn_adapter_saved_transpose_operand")
            if bool(getattr(_x, "_te_rowwise_transpose_for_backward_operand", False)):
                _cppmega_record_bwd_stat(
                    "mxfp8_tn_adapter_te_autograd_saved_operand"
                )
            if bool(getattr(_x, "_cppmega_mxfp8_rowwise_transpose_operand", False)):
                _cppmega_record_bwd_stat(
                    "mxfp8_tn_adapter_fused_producer_operand"
                )
        ...
        def _cppmega_mxfp8_colwise_as_rowwise_transpose(
            ...
            *,
            ...
            if not _ignore_saved and (
                ...
                _cppmega_record_gemm_ready_saved_operand(_x)
                return _x
            _sidecar = None if _ignore_saved else _cppmega_get_mxfp8_sidecar(_x)
            if _sidecar is not None:
                _cppmega_mark_rowwise_transpose_operand(_sidecar)
                _cppmega_record_bwd_stat(
                    "mxfp8_tn_adapter_registry_sidecar_operand"
                )
            if _sidecar is not None:
        ...
        def _cppmega_take_gemm_ready_grouped_transpose(_item):
            if _cppmega_is_mxfp8_rowwise_transpose_operand(_item):
                _cppmega_record_gemm_ready_saved_operand(_item)
                return _item
            _sidecar = _cppmega_get_mxfp8_sidecar(_item)
            if _sidecar is None:
                raise ValueError("missing_gemm_ready_grouped_transpose")
            _cppmega_mark_rowwise_transpose_operand(_sidecar)
            _cppmega_record_bwd_stat("mxfp8_tn_adapter_registry_sidecar_operand")
            _cppmega_record_bwd_stat("mxfp8_tn_adapter_te_emit")
            return _sidecar
