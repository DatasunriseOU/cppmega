...
if (
    ...
    try:
        ...
        def _cppmega_mark_linear_contract_v1_gemm_ready_operand(_x):
            try:
                setattr(_x, "_cppmega_linear_contract_v1_gemm_ready_operand", True)
            except Exception:
                pass
            return _x

        def _cppmega_is_linear_contract_v1_gemm_ready_operand(_x):
            return bool(getattr(_x, "_cppmega_linear_contract_v1_gemm_ready_operand", False))

        def _cppmega_record_rowwise_transpose_operand_consumed(_x):
            if _te_mxfp8_linear_gemm_ready_v1 and _cppmega_is_linear_contract_v1_gemm_ready_operand(_x):
                _cppmega_record_bwd_stat(
                    "mxfp8_linear_contract_v1_gemm_ready_operand_consumed"
                )
            else:
                _cppmega_record_bwd_stat("mxfp8_tn_adapter_saved_transpose_operand")

        def _cppmega_record_compact_columnwise_operand_consumed(_x):
            if not (_te_mxfp8_linear_gemm_ready_v1 and _cppmega_is_mxfp8_tensor(_x)):
                return
            _cppmega_record_bwd_stat(
                "mxfp8_linear_contract_v1_compact_columnwise_operand_consumed"
            )
        ...
        def _cppmega_promote_attached_gemm_ready_transpose(_x, *, _record=True):
            ...
            if _record and _te_mxfp8_linear_gemm_ready_v1 and not _contract_excluded:
                _cppmega_mark_linear_contract_v1_gemm_ready_operand(_sidecar)
        ...
        def _cppmega_configure_linear_contract_quantizer(
            ...
            *,
            ...
            setattr(
                ...
                bool(
                    _contract_applies
                    and not _te_mxfp8_linear_gemm_ready_v1_compact_dense
            ...
            setattr(
                ...
                bool(
                    _te_mxfp8_compact_columnwise_backward
                    and (
                        not _contract_applies
                        or _te_mxfp8_linear_gemm_ready_v1_compact_dense
                    )
                ),
            )
        ...
        def _cppmega_mxfp8_colwise_as_rowwise_transpose(
            ...
            *,
            ...
            if not _ignore_saved and (
                ...
                _cppmega_record_rowwise_transpose_operand_consumed(_x)
                return _x
            _sidecar = None if _ignore_saved else _cppmega_get_mxfp8_sidecar(_x)
            if _sidecar is not None:
                _cppmega_mark_rowwise_transpose_operand(_sidecar)
            if _sidecar is not None:
                if _cppmega_is_linear_contract_v1_gemm_ready_operand(_sidecar):
                    _cppmega_record_rowwise_transpose_operand_consumed(_sidecar)
                else:
        ...
        def _cppmega_cutlass_dgrad_nn_gemm(_weight, _dy, _kwargs):
            ...
            _cppmega_record_compact_columnwise_operand_consumed(_weight)
            _cppmega_record_compact_columnwise_operand_consumed(_dy)
            return _result, None, None, None
        ...
        def _cppmega_cutlass_wgrad_nt_gemm(_x, _dy, _kwargs):
            ...
            _cppmega_record_compact_columnwise_operand_consumed(_x)
            _cppmega_record_compact_columnwise_operand_consumed(_dy)
            return _result, None, None, None
        ...
        def _cppmega_take_gemm_ready_grouped_transpose(_item):
            if _cppmega_is_mxfp8_rowwise_transpose_operand(_item):
                _cppmega_record_rowwise_transpose_operand_consumed(_item)
                return _item
            ...
            if _orig_make is not None and not getattr(
                ...
                def _make_rowwise_transpose_for_backward(source, tensor, quantizer):
                    if (
                        ...
                        raise RuntimeError(
                            "MXFP8 Linear gemm_ready_v1 contract requires TE "
                            "Linear/autograd to save compact-columnwise MXFP8 "
                            "operands and route backward through a direct "
                            "backend. TE's separate quantize_rowwise_transpose "
                            "launch is disabled."
                        )
            ...
            if _orig_copy is not None and not getattr(
                ...
                def _copy_columnwise_as_rowwise_transpose_for_backward(tensor):
                    if _te_mxfp8_linear_gemm_ready_v1 and _cppmega_is_mxfp8_tensor(tensor):
                        ...
                        raise RuntimeError(
                            "MXFP8 Linear gemm_ready_v1 contract requires direct "
                            "compact-columnwise MXFP8 backward; the "
                            "copy-columnwise bridge is disabled."
                        )
