...
# scripts/remote_smoke_h200_nam56r_k_pp1.sh for current production work.
...
CPPMEGA_RUN_ID="${CPPMEGA_RUN_ID:-cppmega_nam56r_lite_train}"
REMOTE_VENV="${REMOTE_VENV:-${REMOTE_ROOT}/cppmega-venv}"
REMOTE_LOG="${REMOTE_LOG:-${REMOTE_ROOT}/cppmega/${CPPMEGA_RUN_ID}.log}"
REMOTE_TMP_SCRIPT="${REMOTE_TMP_SCRIPT:-/tmp/cppmega-remote-nam56r-lite-train.sh}"
LOCAL_TMP_SCRIPT="$(mktemp -t cppmega-remote-nam56r-lite.XXXXXX.sh)"
...
REMOTE_WORKDIR="$(mktemp -d /tmp/cppmega-nam56r-lite.XXXXXX)"
...
repo = Path('/mnt/data/megatron-lm')
target = repo / 'megatron/core/models/common/embeddings/language_model_embedding.py'
clean = subprocess.check_output(
    ['git', '-C', str(repo), 'show', 'HEAD:megatron/core/models/common/embeddings/language_model_embedding.py'],
    text=True,
)
if target.read_text() != clean:
...
python -m torch.distributed.run --nproc_per_node=8 "${REMOTE_WORKDIR}/pretrain_mamba.py" \
  ...
  --spec cppmega.megatron.nam56r_lite_spec build_cppmega_nam56r_lite_stack_spec \
  ${NATIVE_ARGS} \
