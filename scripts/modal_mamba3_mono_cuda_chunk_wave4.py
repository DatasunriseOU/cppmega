"""Modal runner for the Wave 4 P64-panel WMMA CUDA schedule prototype."""
...
APP_NAME = "cppmega-mamba3-mono-cuda-chunk-wave4-" + re.sub(r"[^0-9A-Za-z]+", "-", GPU_SPEC).lower()
...
def run_remote(
    ...
    os.environ["RR_DIAG_MONO_P_TILE"] = "32"
    os.environ["RR_DIAG_CUDA_EXT_SUFFIX"] = "mono_wave4_wmma_p64_panel"
    ...
    sys.modules.pop("rr_mono_cuda_chunk_wave2", None)
    sys.modules.pop("rr_mono_cuda_chunk_wave3", None)
    sys.modules.pop("rr_mono_cuda_chunk_wave4", None)

    bench_dir = pathlib.Path(CPPMEGA_ROOT) / "upstream_prs/examples/13_tilelang_floormod_dbz"
    sys.path.insert(0, str(bench_dir))
    path = bench_dir / "rr_mono_cuda_chunk_wave4.py"
    module_name = "rr_mono_cuda_chunk_wave4_" + re.sub(r"[^0-9A-Za-z_]+", "_", shape_csv)
...
def main(
    ...
    if background:
        call = run_remote.spawn(shape_csv, iters, warmup)
        print(f"FUNCTION_CALL_ID={call.object_id}")
        return
    results = run_remote.remote(shape_csv, iters, warmup)
    for result in results:
        print(json.dumps(result, indent=2, sort_keys=True))
    print("SUMMARY_JSON=" + json.dumps(results, sort_keys=True))
