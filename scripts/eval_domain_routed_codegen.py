#!/usr/bin/env python3
"""Local domain-routed codegen eval harness.

This script deliberately does not run remote GPUs. It evaluates completions
already generated by a model, validates frozen prompt domains and sidecars, and
dispatches each task to one explicit fail-closed local oracle contract.
"""

from __future__ import annotations

import argparse
from collections import Counter
from collections.abc import Mapping, Sequence
from dataclasses import dataclass, field
import json
from pathlib import Path
import re
import shutil
import subprocess
import sys
import tempfile
from typing import Any

_ROOT = Path(__file__).resolve().parents[1]
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))

from cppmega.data.build_parsers import (  # noqa: E402
    parse_bazel,
    parse_cmake,
    parse_configure,
    parse_make,
    parse_ninja,
)
from cppmega.data.diagnostic_parsers import (  # noqa: E402
    parse_build_error,
    parse_clang_diagnostic,
    parse_linker_error,
    parse_sanitizer_output,
    parse_test_output,
)
from cppmega.data.domain_schema import (  # noqa: E402
    DOMAIN_EDGE_FIELD_FAMILIES,
    DomainKind,
    DomainRoleKind,
    ParseConfidence,
    delimiter_token_ids,
    domain_edge_family,
)
from cppmega.data.shell_parsers import (  # noqa: E402
    parse_bash,
    parse_ksh,
    parse_sh,
    parse_zsh,
)

_PROMPT_TOKEN_SIDECARS = (
    "token_domain_ids",
    "token_role_ids",
    "token_entity_ids",
    "token_scope_ids",
    "token_source_doc_ids",
    "token_source_identity_ids",
    "token_confidence_ids",
)
_PROMPT_EDGE_SIDECARS = tuple(
    f"token_{field}" for field in DOMAIN_EDGE_FIELD_FAMILIES
)
_PROMPT_EDGE_FAMILIES = {
    f"token_{field}": family
    for field, family in DOMAIN_EDGE_FIELD_FAMILIES.items()
}
_DOMAIN_NAME_ALIASES = {"CPP_CODE": "CPP"}

ORACLE_CPP_COMPILE_RUN = "cpp_compile_run"
ORACLE_SHELL_SYNTAX = "shell_syntax"
ORACLE_BUILD_STRUCTURE = "build_structure"
ORACLE_SIDECAR_STRUCTURE = "sidecar_structure"
ORACLE_DIAGNOSTIC_STRUCTURE = "diagnostic_structure"
ORACLE_KINDS = frozenset(
    {
        ORACLE_CPP_COMPILE_RUN,
        ORACLE_SHELL_SYNTAX,
        ORACLE_BUILD_STRUCTURE,
        ORACLE_SIDECAR_STRUCTURE,
        ORACLE_DIAGNOSTIC_STRUCTURE,
    }
)

_SHELL_ORACLE_SPECS = {
    DomainKind.BASH: ("bash", parse_bash),
    DomainKind.ZSH: ("zsh", parse_zsh),
    DomainKind.KSH: ("ksh", parse_ksh),
    DomainKind.SH: ("sh", parse_sh),
}
_BUILD_ORACLE_SPECS = {
    DomainKind.CMAKE: parse_cmake,
    DomainKind.MAKE: parse_make,
    DomainKind.NINJA: parse_ninja,
    DomainKind.BAZEL: parse_bazel,
    DomainKind.CONFIGURE: parse_configure,
}
_DIAGNOSTIC_ORACLE_SPECS = {
    DomainKind.COMPILER_DIAGNOSTIC: ("compiler", parse_clang_diagnostic),
    DomainKind.COMPILER_ERROR: ("compiler", parse_clang_diagnostic),
    DomainKind.BUILD_DIAGNOSTIC: ("build", parse_build_error),
    DomainKind.BUILD_ERROR: ("build", parse_build_error),
    DomainKind.LINKER_ERROR: ("linker", parse_linker_error),
    DomainKind.LINKER_DIAGNOSTIC: ("linker", parse_linker_error),
    DomainKind.TEST_OUTPUT: ("test", parse_test_output),
    DomainKind.SANITIZER_OUTPUT: ("sanitizer", parse_sanitizer_output),
}
_DIAGNOSTIC_DOMAINS = frozenset(
    {
        DomainKind.COMPILER_DIAGNOSTIC,
        DomainKind.BUILD_DIAGNOSTIC,
        DomainKind.COMPILER_ERROR,
        DomainKind.BUILD_ERROR,
        DomainKind.LINKER_ERROR,
        DomainKind.TEST_OUTPUT,
        DomainKind.TOOL_OUTPUT,
        DomainKind.LINKER_DIAGNOSTIC,
        DomainKind.SANITIZER_OUTPUT,
    }
)
_DOMAIN_MARKER_RE = re.compile(r"<([A-Z][A-Z0-9_]*?)_(START|END)>")


def _prompt_domain_id(name: str, *, context: str) -> int:
    canonical = _DOMAIN_NAME_ALIASES.get(name.upper(), name.upper())
    try:
        return int(DomainKind[canonical])
    except KeyError as exc:
        raise ValueError(f"{context}: unknown required domain {name!r}") from exc


def _canonical_prompt_domain(name: Any, *, context: str) -> str:
    raw = str(name).upper()
    canonical = _DOMAIN_NAME_ALIASES.get(raw, raw)
    try:
        return DomainKind[canonical].name
    except KeyError as exc:
        raise ValueError(f"{context}: unknown required domain {name!r}") from exc


def _prompt_domain_kinds(
    required_domains: Sequence[str],
    *,
    context: str,
) -> tuple[DomainKind, ...]:
    return tuple(
        DomainKind(_prompt_domain_id(name, context=context))
        for name in required_domains
    )


def _validate_prompt_domain_markers(
    prompt: str,
    required_domains: Sequence[str],
    *,
    context: str,
) -> None:
    required = set(required_domains)
    stack: list[str] = []
    for match in _DOMAIN_MARKER_RE.finditer(prompt):
        domain = _canonical_prompt_domain(match.group(1), context=context)
        if domain not in required:
            raise ValueError(
                f"{context}: prompt delimiter domain {domain!r} is absent from "
                "required_domains"
            )
        if match.group(2) == "START":
            stack.append(domain)
        elif not stack or stack.pop() != domain:
            raise ValueError(f"{context}: unmatched prompt domain end delimiter {domain}")
    if stack:
        raise ValueError(f"{context}: unclosed prompt domain delimiters {stack}")


def _required_prompt_sidecars(
    required_domains: Sequence[str],
    *,
    context: str,
) -> frozenset[str]:
    domains = set(_prompt_domain_kinds(required_domains, context=context))
    required = {"token_domain_ids", "token_role_ids"}
    if domains.intersection(_SHELL_ORACLE_SPECS):
        required.add("token_shell_edges")
    build_domains = domains.intersection(_BUILD_ORACLE_SPECS)
    if build_domains:
        if build_domains == {DomainKind.CONFIGURE}:
            required.add("token_shell_edges")
        else:
            required.add("token_build_edges")
    if domains.intersection(_DIAGNOSTIC_DOMAINS):
        required.add("token_diagnostic_edges")
    if len(domains) > 1:
        required.add("token_cross_domain_edges")
    return frozenset(required)


def _validate_expected_sidecars(
    required_domains: Sequence[str],
    expected_sidecars: Sequence[str],
    *,
    context: str,
) -> None:
    if len(set(expected_sidecars)) != len(expected_sidecars):
        raise ValueError(f"{context}: expected_sidecars must not contain duplicates")
    missing = sorted(
        _required_prompt_sidecars(required_domains, context=context)
        - set(expected_sidecars)
    )
    if missing:
        raise ValueError(
            f"{context}: required domains need expected sidecars {missing}"
        )


def _prompt_delimiter_domains() -> dict[int, int]:
    result: dict[int, int] = {}
    for domain in DomainKind:
        if domain == DomainKind.UNKNOWN:
            continue
        try:
            start, end = delimiter_token_ids(domain)
        except KeyError:
            continue
        result[int(start)] = int(domain)
        result[int(end)] = int(domain)
    return result


_PROMPT_DELIMITER_DOMAINS = _prompt_delimiter_domains()


def _prompt_start_delimiter_ids() -> frozenset[int]:
    result: set[int] = set()
    for domain in DomainKind:
        if domain == DomainKind.UNKNOWN:
            continue
        try:
            start, _end = delimiter_token_ids(domain)
        except KeyError:
            continue
        result.add(int(start))
    return frozenset(result)


_PROMPT_START_DELIMITER_IDS = _prompt_start_delimiter_ids()


def _normalize_prompt_edge(
    edge: Any,
    *,
    column: str,
    token_count: int,
    context: str,
) -> dict[str, int]:
    if not isinstance(edge, Mapping):
        raise ValueError(f"{context}: {column} entries must be objects")
    src = edge.get("from", edge.get("src"))
    dst = edge.get("to", edge.get("dst"))
    if src is None or dst is None or edge.get("kind") is None:
        raise ValueError(f"{context}: {column} entries require from/to/kind")
    try:
        normalized = {"from": int(src), "to": int(dst), "kind": int(edge["kind"])}
    except (TypeError, ValueError) as exc:
        raise ValueError(f"{context}: {column} edge values must be integers") from exc
    if not (
        0 <= normalized["from"] < token_count
        and 0 <= normalized["to"] < token_count
    ):
        raise ValueError(f"{context}: {column} edge endpoint is outside prompt tokens")
    try:
        actual_family = domain_edge_family(normalized["kind"])
    except ValueError as exc:
        raise ValueError(f"{context}: {column}: {exc}") from exc
    expected_family = _PROMPT_EDGE_FAMILIES[column]
    if actual_family != expected_family:
        raise ValueError(
            f"{context}: {column} edge kind {normalized['kind']} belongs to "
            f"{actual_family}, not {expected_family}"
        )
    return normalized


def validate_prompt_sidecars(
    token_ids: Sequence[Any],
    sidecars: Mapping[str, Sequence[Any]],
    required_domains: Sequence[str],
    expected_sidecars: Sequence[str],
    *,
    context: str = "prompt",
) -> None:
    """Validate a frozen prompt token/sidecar snapshot without model execution."""
    token_values = [int(value) for value in token_ids]
    if not token_values:
        raise ValueError(f"{context}: prompt_token_ids must be non-empty")
    missing = [name for name in expected_sidecars if name not in sidecars]
    if missing:
        raise ValueError(f"{context}: expected sidecars are missing {missing}")
    missing_token = [name for name in _PROMPT_TOKEN_SIDECARS if name not in sidecars]
    if missing_token:
        raise ValueError(
            f"{context}: structured prompt sidecars require all token-aligned "
            f"columns; missing={missing_token}"
        )

    token_count = len(token_values)
    vectors: dict[str, list[int]] = {}
    for column in _PROMPT_TOKEN_SIDECARS:
        try:
            vector = [int(value) for value in sidecars[column]]
        except (TypeError, ValueError) as exc:
            raise ValueError(f"{context}: {column} must be an integer sequence") from exc
        if len(vector) != token_count:
            raise ValueError(
                f"{context}: {column} length {len(vector)} != token_ids length {token_count}"
            )
        vectors[column] = vector

    for value in vectors["token_domain_ids"]:
        try:
            DomainKind(value)
        except ValueError as exc:
            raise ValueError(f"{context}: unknown token_domain_ids value {value}") from exc
    for value in vectors["token_role_ids"]:
        try:
            DomainRoleKind(value)
        except ValueError as exc:
            raise ValueError(f"{context}: unknown token_role_ids value {value}") from exc
    for value in vectors["token_confidence_ids"]:
        try:
            ParseConfidence(value)
        except ValueError as exc:
            raise ValueError(f"{context}: unknown token_confidence_ids value {value}") from exc
    for column in ("token_entity_ids", "token_scope_ids", "token_source_doc_ids"):
        if any(value < 0 for value in vectors[column]):
            raise ValueError(f"{context}: {column} values must be non-negative")
    if any(value <= 0 for value in vectors["token_source_doc_ids"]):
        raise ValueError(f"{context}: token_source_doc_ids values must be positive")
    if any(value <= 0 for value in vectors["token_source_identity_ids"]):
        raise ValueError(f"{context}: token_source_identity_ids values must be positive")

    normalized_edges: dict[str, list[dict[str, int]]] = {
        column: [] for column in _PROMPT_EDGE_SIDECARS
    }
    for column in _PROMPT_EDGE_SIDECARS:
        raw_edges = sidecars.get(column, ())
        if not isinstance(raw_edges, Sequence) or isinstance(raw_edges, (str, bytes)):
            raise ValueError(f"{context}: {column} must be an edge sequence")
        for edge in raw_edges:
            normalized_edges[column].append(
                _normalize_prompt_edge(
                    edge,
                    column=column,
                    token_count=token_count,
                    context=context,
                )
            )

    represented_domains = set(vectors["token_domain_ids"])
    for name in required_domains:
        domain_id = _prompt_domain_id(str(name), context=context)
        if domain_id not in represented_domains:
            raise ValueError(
                f"{context}: required domain {name!r} is absent from token_domain_ids"
            )

    diagnostic_domain_ids = {int(domain) for domain in _DIAGNOSTIC_DOMAINS}
    for edge in normalized_edges["token_diagnostic_edges"]:
        if (
            vectors["token_domain_ids"][edge["from"]] not in diagnostic_domain_ids
            and vectors["token_domain_ids"][edge["to"]] not in diagnostic_domain_ids
        ):
            raise ValueError(
                f"{context}: token_diagnostic_edges must touch a diagnostic domain"
            )
    for edge in normalized_edges["token_cross_domain_edges"]:
        source_domain = vectors["token_domain_ids"][edge["from"]]
        target_domain = vectors["token_domain_ids"][edge["to"]]
        if source_domain == target_domain:
            raise ValueError(
                f"{context}: token_cross_domain_edges must connect distinct domain ids"
            )

    stack: list[int] = []
    for index, token_id in enumerate(token_values):
        domain_id = vectors["token_domain_ids"][index]
        role_id = vectors["token_role_ids"][index]
        confidence_id = vectors["token_confidence_ids"][index]
        delimiter_domain = _PROMPT_DELIMITER_DOMAINS.get(token_id)
        if delimiter_domain is not None:
            if domain_id != delimiter_domain:
                raise ValueError(
                    f"{context}: delimiter token {token_id} has domain id {domain_id}, "
                    f"expected {delimiter_domain}"
                )
            if role_id != int(DomainRoleKind.DELIMITER):
                raise ValueError(f"{context}: delimiter token {token_id} must have role DELIMITER")
            if confidence_id != int(ParseConfidence.EXACT):
                raise ValueError(f"{context}: delimiter token {token_id} must have EXACT confidence")
            if token_id in _PROMPT_START_DELIMITER_IDS:
                stack.append(delimiter_domain)
            elif not stack or stack.pop() != delimiter_domain:
                raise ValueError(f"{context}: unmatched domain end delimiter {token_id}")
            continue
        if role_id == int(DomainRoleKind.DELIMITER):
            raise ValueError(f"{context}: non-delimiter token {token_id} has DELIMITER role")
        active_domain = stack[-1] if stack else int(DomainKind.UNKNOWN)
        if domain_id != active_domain:
            raise ValueError(
                f"{context}: token {index} domain id {domain_id} does not match "
                f"active domain {active_domain}"
            )
    if stack:
        raise ValueError(f"{context}: unclosed domain delimiters {stack}")


def _prompt_sidecar_receipt(
    token_ids: Sequence[int],
    sidecars: Mapping[str, Sequence[Any]],
) -> dict[str, Any]:
    return {
        "token_count": len(token_ids),
        "columns": sorted(sidecars),
        "edge_counts": {
            column: len(sidecars.get(column, ()))
            for column in _PROMPT_EDGE_SIDECARS
            if column in sidecars
        },
    }


PASS_STATUS = "compile_passed"
PASS_STATUSES = frozenset(
    {
        "compile_passed",
        "shell_syntax_passed",
        "build_structure_passed",
        "sidecar_structure_passed",
        "diagnostic_structure_passed",
    }
)
FAIL_STATUSES = frozenset(
    {
        "missing_completion",
        "not_compiled",
        "missing_compile_oracle",
        "missing_shell_oracle",
        "missing_build_oracle",
        "missing_sidecar_oracle",
        "missing_diagnostic_oracle",
        "compile_oracle_unavailable",
        "shell_oracle_unavailable",
        "build_oracle_unavailable",
        "sidecar_oracle_unavailable",
        "compile_timeout",
        "compile_failed",
        "runtime_timeout",
        "runtime_failed",
        "shell_syntax_timeout",
        "shell_syntax_failed",
        "shell_parse_failed",
        "build_structure_failed",
        "sidecar_structure_failed",
        "diagnostic_structure_failed",
    }
)


@dataclass(frozen=True)
class DomainEvalPrompt:
    id: str
    task_type: str
    prompt: str
    required_domains: tuple[str, ...]
    expected_sidecars: tuple[str, ...]
    oracle_kind: str = ORACLE_CPP_COMPILE_RUN
    compile_prefix: str = ""
    compile_suffix: str = ""
    run_binary: bool = False
    prompt_token_ids: tuple[int, ...] = ()
    prompt_sidecars: Mapping[str, tuple[Any, ...]] = field(default_factory=dict)
    oracle_edge_kinds: tuple[int, ...] = ()
    oracle_min_edges: int = 0

    def oracle_contract_error(self) -> str | None:
        if self.oracle_kind not in ORACLE_KINDS:
            return f"unknown oracle_kind {self.oracle_kind!r}"
        try:
            domains = set(
                _prompt_domain_kinds(
                    self.required_domains,
                    context=f"prompt {self.id!r}",
                )
            )
        except ValueError as exc:
            return str(exc)
        if self.oracle_kind == ORACLE_CPP_COMPILE_RUN:
            if DomainKind.CPP not in domains:
                return "cpp_compile_run oracle requires required_domains to include CPP"
            if not (self.compile_prefix.strip() or self.compile_suffix.strip()):
                return "compile_prefix or compile_suffix must provide a test wrapper"
            if not self.run_binary:
                return "run_binary must be true so compiled output is executed"
            return None
        if self.compile_prefix.strip() or self.compile_suffix.strip() or self.run_binary:
            return (
                f"{self.oracle_kind} oracle cannot declare compile_prefix, "
                "compile_suffix, or run_binary"
            )
        if self.oracle_kind == ORACLE_SHELL_SYNTAX:
            shell_domains = domains.intersection(_SHELL_ORACLE_SPECS)
            if len(shell_domains) != 1:
                return (
                    "shell_syntax oracle requires exactly one shell domain "
                    "(BASH, ZSH, KSH, or SH)"
                )
            return None
        if self.oracle_kind == ORACLE_BUILD_STRUCTURE:
            build_domains = domains.intersection(_BUILD_ORACLE_SPECS)
            if len(build_domains) != 1:
                return (
                    "build_structure oracle requires exactly one build/config "
                    "domain (CMAKE, MAKE, NINJA, BAZEL, or CONFIGURE)"
                )
            return None
        if self.oracle_kind == ORACLE_DIAGNOSTIC_STRUCTURE:
            diagnostic_domains = domains.intersection(_DIAGNOSTIC_ORACLE_SPECS)
            if len(diagnostic_domains) != 1:
                return (
                    "diagnostic_structure oracle requires exactly one diagnostic "
                    "domain"
                )
            if self.oracle_min_edges < 1 and not self.oracle_edge_kinds:
                return (
                    "diagnostic_structure oracle requires an expected diagnostic "
                    "edge kind or positive oracle_min_edges"
                )
            return None
        if not self.prompt_token_ids or not self.prompt_sidecars:
            return "sidecar_structure oracle requires frozen prompt_token_ids and prompt_sidecars"
        if not (
            domains.intersection(_DIAGNOSTIC_DOMAINS)
            or "token_cross_domain_edges" in self.expected_sidecars
        ):
            return (
                "sidecar_structure oracle requires a diagnostic domain or "
                "cross-domain sidecar"
            )
        return None

    def executable_oracle_error(self) -> str | None:
        """Compatibility name for callers that specifically request C++ execution."""

        if self.oracle_kind != ORACLE_CPP_COMPILE_RUN:
            return f"oracle_kind {self.oracle_kind!r} is not cpp_compile_run"
        return self.oracle_contract_error()

    def sidecar_receipt(self) -> dict[str, Any]:
        return _prompt_sidecar_receipt(self.prompt_token_ids, self.prompt_sidecars)

    @classmethod
    def from_row(
        cls, row: dict[str, Any], *, path: Path, line_no: int
    ) -> "DomainEvalPrompt":
        missing = [
            key
            for key in (
                "id",
                "task_type",
                "prompt",
                "required_domains",
                "oracle_kind",
            )
            if key not in row
        ]
        if missing:
            raise ValueError(
                f"{path}:{line_no}: missing required prompt fields {missing}"
            )
        raw_required_domains = row["required_domains"]
        if not isinstance(raw_required_domains, Sequence) or isinstance(
            raw_required_domains, (str, bytes)
        ):
            raise ValueError(f"{path}:{line_no}: required_domains must be a sequence")
        required_domains = tuple(
            _canonical_prompt_domain(x, context=f"{path}:{line_no}")
            for x in raw_required_domains
        )
        if not required_domains:
            raise ValueError(f"{path}:{line_no}: required_domains must be non-empty")
        if len(set(required_domains)) != len(required_domains):
            raise ValueError(f"{path}:{line_no}: required_domains must not contain duplicates")
        raw_expected_sidecars = row.get("expected_sidecars", ())
        if not isinstance(raw_expected_sidecars, Sequence) or isinstance(
            raw_expected_sidecars, (str, bytes)
        ):
            raise ValueError(f"{path}:{line_no}: expected_sidecars must be a sequence")
        expected_sidecars = tuple(str(x) for x in raw_expected_sidecars)
        _validate_expected_sidecars(
            required_domains,
            expected_sidecars,
            context=f"{path}:{line_no}",
        )
        oracle_kind = row["oracle_kind"]
        if not isinstance(oracle_kind, str):
            raise ValueError(f"{path}:{line_no}: oracle_kind must be a string")
        if oracle_kind not in ORACLE_KINDS:
            raise ValueError(
                f"{path}:{line_no}: unknown oracle_kind {oracle_kind!r}; "
                f"expected one of {sorted(ORACLE_KINDS)}"
            )
        _validate_prompt_domain_markers(
            str(row["prompt"]),
            required_domains,
            context=f"{path}:{line_no}",
        )
        raw_edge_kinds = row.get("oracle_edge_kinds", ())
        if not isinstance(raw_edge_kinds, Sequence) or isinstance(
            raw_edge_kinds, (str, bytes)
        ):
            raise ValueError(f"{path}:{line_no}: oracle_edge_kinds must be a sequence")
        try:
            oracle_edge_kinds = tuple(int(value) for value in raw_edge_kinds)
        except (TypeError, ValueError) as exc:
            raise ValueError(
                f"{path}:{line_no}: oracle_edge_kinds must contain integers"
            ) from exc
        if len(set(oracle_edge_kinds)) != len(oracle_edge_kinds):
            raise ValueError(f"{path}:{line_no}: oracle_edge_kinds must not contain duplicates")
        try:
            oracle_min_edges = int(row.get("oracle_min_edges", 0))
        except (TypeError, ValueError) as exc:
            raise ValueError(f"{path}:{line_no}: oracle_min_edges must be an integer") from exc
        if oracle_min_edges < 0:
            raise ValueError(f"{path}:{line_no}: oracle_min_edges must be non-negative")
        raw_token_ids = row.get("prompt_token_ids")
        raw_sidecars = row.get("prompt_sidecars")
        if (raw_token_ids is None) != (raw_sidecars is None):
            raise ValueError(
                f"{path}:{line_no}: prompt_token_ids and prompt_sidecars must be "
                "provided together"
            )
        prompt_token_ids: tuple[int, ...] = ()
        prompt_sidecars: dict[str, tuple[Any, ...]] = {}
        if raw_token_ids is not None:
            if not isinstance(raw_token_ids, Sequence) or isinstance(
                raw_token_ids, (str, bytes)
            ):
                raise ValueError(
                    f"{path}:{line_no}: prompt_token_ids must be an integer sequence"
                )
            if not isinstance(raw_sidecars, Mapping):
                raise ValueError(f"{path}:{line_no}: prompt_sidecars must be an object")
            prompt_token_ids = tuple(int(value) for value in raw_token_ids)
            for column, values in raw_sidecars.items():
                if not isinstance(values, Sequence) or isinstance(values, (str, bytes)):
                    raise ValueError(
                        f"{path}:{line_no}: prompt sidecar {column!r} must be a sequence"
                    )
                column_name = str(column)
                if column_name in _PROMPT_EDGE_SIDECARS:
                    prompt_sidecars[column_name] = tuple(
                        _normalize_prompt_edge(
                            edge,
                            column=column_name,
                            token_count=len(prompt_token_ids),
                            context=f"{path}:{line_no}",
                        )
                        for edge in values
                    )
                else:
                    prompt_sidecars[column_name] = tuple(values)
            validate_prompt_sidecars(
                prompt_token_ids,
                prompt_sidecars,
                required_domains,
                expected_sidecars,
                context=f"{path}:{line_no}",
            )
        prompt = cls(
            id=str(row["id"]),
            task_type=str(row["task_type"]),
            prompt=str(row["prompt"]),
            required_domains=required_domains,
            expected_sidecars=expected_sidecars,
            oracle_kind=oracle_kind,
            compile_prefix=str(row.get("compile_prefix", "")),
            compile_suffix=str(row.get("compile_suffix", "")),
            run_binary=bool(row.get("run_binary", False)),
            prompt_token_ids=prompt_token_ids,
            prompt_sidecars=prompt_sidecars,
            oracle_edge_kinds=oracle_edge_kinds,
            oracle_min_edges=oracle_min_edges,
        )
        oracle_error = prompt.oracle_contract_error()
        if oracle_error is not None:
            raise ValueError(
                f"{path}:{line_no}: prompt {prompt.id!r} has invalid "
                f"{prompt.oracle_kind} oracle contract: {oracle_error}"
            )
        return prompt


def load_prompts(path: Path) -> list[DomainEvalPrompt]:
    prompts: list[DomainEvalPrompt] = []
    seen: set[str] = set()
    with path.open("r", encoding="utf-8") as fh:
        for line_no, line in enumerate(fh, 1):
            line = line.strip()
            if not line:
                continue
            row = json.loads(line)
            prompt = DomainEvalPrompt.from_row(row, path=path, line_no=line_no)
            if prompt.id in seen:
                raise ValueError(f"{path}:{line_no}: duplicate prompt id {prompt.id!r}")
            seen.add(prompt.id)
            prompts.append(prompt)
    if not prompts:
        raise ValueError(f"{path}: no prompts")
    return prompts


def load_completions(path: Path | None) -> dict[str, str]:
    if path is None:
        return {}
    completions: dict[str, str] = {}
    with path.open("r", encoding="utf-8") as fh:
        for line_no, line in enumerate(fh, 1):
            line = line.strip()
            if not line:
                continue
            row = json.loads(line)
            if "id" not in row or "completion" not in row:
                raise ValueError(
                    f"{path}:{line_no}: completion rows need id and completion"
                )
            completion_id = str(row["id"])
            if completion_id in completions:
                raise ValueError(
                    f"{path}:{line_no}: duplicate completion id {completion_id!r}"
                )
            completions[completion_id] = str(row["completion"])
    return completions


ToolOverrides = Mapping[str, str | None]


def _resolve_tool(
    logical_name: str,
    candidates: Sequence[str],
    tool_overrides: ToolOverrides | None,
) -> str | None:
    if tool_overrides is not None and logical_name in tool_overrides:
        return tool_overrides[logical_name]
    for name in candidates:
        path = shutil.which(name)
        if path:
            return path
    return None


def _find_cpp_compiler(tool_overrides: ToolOverrides | None = None) -> str | None:
    return _resolve_tool("cpp", ("clang++", "c++", "g++"), tool_overrides)


def _parsed_receipt(parsed: Any) -> dict[str, Any]:
    return {
        "parser_adapter": parsed.metadata.get("parser_adapter"),
        "token_count": len(parsed.tokens),
        "edge_count": len(parsed.edges),
        "edge_kinds": sorted({int(kind) for _src, _dst, kind in parsed.edges}),
        "role_kinds": sorted({int(role) for role in parsed.role_ids}),
        "metadata": {
            key: value
            for key, value in parsed.metadata.items()
            if not key.startswith("_")
        },
        "confidence_ids": sorted(set(int(value) for value in parsed.confidence_ids)),
    }


def _single_domain(
    prompt: DomainEvalPrompt,
    allowed: Mapping[DomainKind, Any],
) -> DomainKind | None:
    domains = set(_prompt_domain_kinds(prompt.required_domains, context=prompt.id))
    selected = domains.intersection(allowed)
    return next(iter(selected)) if len(selected) == 1 else None


def _failure(status: str, reason: str, **extra: Any) -> dict[str, Any]:
    return {
        "status": status,
        "reason": reason,
        "failed_closed": True,
        **extra,
    }


def _edge_contract_failure(
    prompt: DomainEvalPrompt,
    parsed: Any,
    *,
    status: str,
    family: str,
) -> dict[str, Any] | None:
    edge_kinds = {int(kind) for _src, _dst, kind in parsed.edges}
    if len(parsed.edges) < prompt.oracle_min_edges:
        return _failure(
            status,
            f"oracle requires at least {prompt.oracle_min_edges} {family} edges, "
            f"got {len(parsed.edges)}",
            **_parsed_receipt(parsed),
        )
    for kind in prompt.oracle_edge_kinds:
        try:
            actual_family = domain_edge_family(kind)
        except ValueError as exc:
            return _failure(status, str(exc), **_parsed_receipt(parsed))
        if actual_family != family:
            return _failure(
                status,
                f"expected edge kind {kind} belongs to {actual_family}, not {family}",
                **_parsed_receipt(parsed),
            )
        if kind not in edge_kinds:
            return _failure(
                status,
                f"oracle is missing required edge kind {kind}",
                **_parsed_receipt(parsed),
            )
    return None


def compile_cpp_completion(
    prompt: DomainEvalPrompt,
    completion: str,
    *,
    compiler: str | None = None,
    compile_timeout_s: float = 120.0,
    run_timeout_s: float = 10.0,
    tool_overrides: ToolOverrides | None = None,
) -> dict[str, Any]:
    oracle_error = prompt.executable_oracle_error()
    if oracle_error is not None:
        return _failure("missing_compile_oracle", oracle_error)
    compiler = compiler if compiler is not None else _find_cpp_compiler(tool_overrides)
    if compiler is None:
        return _failure(
            "compile_oracle_unavailable",
            "no local C++ compiler found",
        )
    source = prompt.compile_prefix + completion + prompt.compile_suffix
    with tempfile.TemporaryDirectory(prefix="cppmega-domain-eval-") as tmp:
        src = Path(tmp) / "main.cpp"
        exe = Path(tmp) / "a.out"
        src.write_text(source, encoding="utf-8")
        try:
            proc = subprocess.run(
                [compiler, "-std=c++20", "-Wall", "-Wextra", str(src), "-o", str(exe)],
                capture_output=True,
                text=True,
                timeout=compile_timeout_s,
            )
        except subprocess.TimeoutExpired as exc:
            return _failure(
                "compile_timeout",
                "C++ compiler exceeded the compile timeout",
                timeout_s=compile_timeout_s,
                stderr=str(exc.stderr or "")[-4000:],
            )
        except OSError as exc:
            return _failure("compile_oracle_unavailable", str(exc))
        if proc.returncode != 0:
            return _failure(
                "compile_failed",
                "C++ compilation failed",
                returncode=proc.returncode,
                stderr=proc.stderr[-4000:],
            )
        try:
            run = subprocess.run(
                [str(exe)],
                capture_output=True,
                text=True,
                timeout=run_timeout_s,
            )
        except subprocess.TimeoutExpired as exc:
            return _failure(
                "runtime_timeout",
                "compiled C++ completion exceeded the runtime timeout",
                returncode=proc.returncode,
                timeout_s=run_timeout_s,
                stderr=proc.stderr[-4000:],
                runtime_stdout=str(exc.stdout or "")[-4000:],
                runtime_stderr=str(exc.stderr or "")[-4000:],
            )
        except OSError as exc:
            return _failure(
                "runtime_failed",
                str(exc),
                returncode=proc.returncode,
                stderr=proc.stderr[-4000:],
            )
        if run.returncode != 0:
            return {
                "status": "runtime_failed",
                "reason": "compiled C++ completion returned a non-zero status",
                "failed_closed": True,
                "returncode": proc.returncode,
                "runtime_returncode": run.returncode,
                "stderr": proc.stderr[-4000:],
                "runtime_stdout": run.stdout[-4000:],
                "runtime_stderr": run.stderr[-4000:],
            }
        return {
            "status": PASS_STATUS,
            "returncode": proc.returncode,
            "runtime_returncode": run.returncode,
            "stderr": proc.stderr[-4000:],
            "runtime_stdout": run.stdout[-4000:],
            "runtime_stderr": run.stderr[-4000:],
        }


def shell_syntax_oracle(
    prompt: DomainEvalPrompt,
    completion: str,
    *,
    shell_executable: str | None = None,
    syntax_timeout_s: float = 10.0,
    tool_overrides: ToolOverrides | None = None,
) -> dict[str, Any]:
    oracle_error = prompt.oracle_contract_error()
    if oracle_error is not None:
        return _failure("missing_shell_oracle", oracle_error)
    domain = _single_domain(prompt, _SHELL_ORACLE_SPECS)
    if domain is None:
        return _failure(
            "missing_shell_oracle",
            "shell_syntax oracle has no single declared shell domain",
        )
    shell_name, parser = _SHELL_ORACLE_SPECS[domain]
    try:
        parsed = parser(completion)
        parsed.validate()
    except (IndexError, TypeError, ValueError) as exc:
        return _failure(
            "shell_parse_failed",
            f"{shell_name} structural parser failed: {exc}",
            shell_kind=shell_name,
        )
    receipt = _parsed_receipt(parsed)
    receipt["shell_kind"] = shell_name
    if parsed.metadata.get("unsupported_syntax") or set(parsed.confidence_ids) == {
        int(ParseConfidence.RAW)
    }:
        return _failure(
            "shell_parse_failed",
            str(parsed.metadata.get("unsupported_syntax", "parser marked syntax RAW")),
            **receipt,
        )
    executable = (
        shell_executable
        if shell_executable is not None
        else _resolve_tool(shell_name, (shell_name,), tool_overrides)
    )
    if executable is None:
        return _failure(
            "shell_oracle_unavailable",
            f"no local {shell_name} executable found",
            **receipt,
        )
    with tempfile.TemporaryDirectory(prefix="cppmega-shell-eval-") as tmp:
        source = Path(tmp) / f"completion.{shell_name}"
        source.write_text(completion, encoding="utf-8")
        try:
            proc = subprocess.run(
                [executable, "-n", str(source)],
                capture_output=True,
                text=True,
                timeout=syntax_timeout_s,
            )
        except subprocess.TimeoutExpired as exc:
            return _failure(
                "shell_syntax_timeout",
                f"{shell_name} syntax check exceeded the timeout",
                timeout_s=syntax_timeout_s,
                stderr=str(exc.stderr or "")[-4000:],
                **receipt,
            )
        except OSError as exc:
            return _failure("shell_oracle_unavailable", str(exc), **receipt)
    if proc.returncode != 0:
        return _failure(
            "shell_syntax_failed",
            f"{shell_name} rejected the completion syntax",
            returncode=proc.returncode,
            stderr=proc.stderr[-4000:],
            stdout=proc.stdout[-4000:],
            **receipt,
        )
    return {
        "status": "shell_syntax_passed",
        "shell_kind": shell_name,
        "executable": executable,
        "stderr": proc.stderr[-4000:],
        "stdout": proc.stdout[-4000:],
        **receipt,
    }


_BUILD_ROLE_REQUIREMENTS: dict[DomainKind, frozenset[DomainRoleKind]] = {
    DomainKind.CMAKE: frozenset({DomainRoleKind.COMMAND}),
    DomainKind.MAKE: frozenset({DomainRoleKind.TARGET, DomainRoleKind.COMMAND}),
    DomainKind.NINJA: frozenset({DomainRoleKind.RULE, DomainRoleKind.OUTPUT}),
    DomainKind.BAZEL: frozenset({DomainRoleKind.RULE, DomainRoleKind.TARGET}),
    DomainKind.CONFIGURE: frozenset(
        {DomainRoleKind.COMMAND, DomainRoleKind.KEYWORD, DomainRoleKind.ENVIRONMENT}
    ),
}


def build_structure_oracle(
    prompt: DomainEvalPrompt,
    completion: str,
) -> dict[str, Any]:
    oracle_error = prompt.oracle_contract_error()
    if oracle_error is not None:
        return _failure("missing_build_oracle", oracle_error)
    domain = _single_domain(prompt, _BUILD_ORACLE_SPECS)
    if domain is None:
        return _failure(
            "missing_build_oracle",
            "build_structure oracle has no single declared build/config domain",
        )
    parser = _BUILD_ORACLE_SPECS[domain]
    try:
        parsed = parser(completion)
        parsed.validate()
    except (IndexError, TypeError, ValueError) as exc:
        return _failure(
            "build_structure_failed",
            f"{domain.name.lower()} structural parser failed: {exc}",
            build_kind=domain.name.lower(),
        )
    receipt = _parsed_receipt(parsed)
    receipt["build_kind"] = domain.name.lower()
    required_roles = _BUILD_ROLE_REQUIREMENTS[domain]
    role_ids = {int(value) for value in parsed.role_ids}
    if parsed.metadata.get("unsupported_syntax") or set(parsed.confidence_ids) == {
        int(ParseConfidence.RAW)
    }:
        return _failure(
            "build_structure_failed",
            str(parsed.metadata.get("unsupported_syntax", "parser marked structure RAW")),
            **receipt,
        )
    if not role_ids.intersection(int(role) for role in required_roles):
        return _failure(
            "build_structure_failed",
            f"{domain.name.lower()} completion has no recognized structural roles",
            **receipt,
        )
    edge_failure = _edge_contract_failure(
        prompt,
        parsed,
        status="build_structure_failed",
        family="shell" if domain == DomainKind.CONFIGURE else "build",
    )
    if edge_failure is not None:
        return edge_failure
    return {
        "status": "build_structure_passed",
        **receipt,
    }


def diagnostic_structure_oracle(
    prompt: DomainEvalPrompt,
    completion: str,
) -> dict[str, Any]:
    oracle_error = prompt.oracle_contract_error()
    if oracle_error is not None:
        return _failure("missing_diagnostic_oracle", oracle_error)
    domain = _single_domain(prompt, _DIAGNOSTIC_ORACLE_SPECS)
    if domain is None:
        return _failure(
            "missing_diagnostic_oracle",
            "diagnostic_structure oracle has no single declared diagnostic domain",
        )
    diagnostic_kind, parser = _DIAGNOSTIC_ORACLE_SPECS[domain]
    try:
        parsed = parser(completion)
        parsed.validate()
    except (IndexError, TypeError, ValueError) as exc:
        return _failure(
            "diagnostic_structure_failed",
            f"{diagnostic_kind} diagnostic parser failed: {exc}",
            diagnostic_kind=diagnostic_kind,
        )
    receipt = _parsed_receipt(parsed)
    receipt["diagnostic_kind"] = diagnostic_kind
    if parsed.metadata.get("unsupported_syntax") or set(parsed.confidence_ids) == {
        int(ParseConfidence.RAW)
    }:
        return _failure(
            "diagnostic_structure_failed",
            str(parsed.metadata.get("unsupported_syntax", "parser marked diagnostic RAW")),
            **receipt,
        )
    if not parsed.edges:
        return _failure(
            "diagnostic_structure_failed",
            f"{diagnostic_kind} diagnostic has no typed diagnostic edges",
            **receipt,
        )
    edge_failure = _edge_contract_failure(
        prompt,
        parsed,
        status="diagnostic_structure_failed",
        family="diagnostic",
    )
    if edge_failure is not None:
        return edge_failure
    return {
        "status": "diagnostic_structure_passed",
        **receipt,
    }


def sidecar_structure_oracle(
    prompt: DomainEvalPrompt,
    completion: str,
) -> dict[str, Any]:
    del completion
    if prompt.oracle_kind != ORACLE_SIDECAR_STRUCTURE:
        return _failure(
            "missing_sidecar_oracle",
            f"oracle_kind {prompt.oracle_kind!r} is not sidecar_structure",
        )
    if not prompt.prompt_token_ids or not prompt.prompt_sidecars:
        return _failure(
            "missing_sidecar_oracle",
            "sidecar_structure oracle requires frozen prompt_token_ids and prompt_sidecars",
        )
    try:
        validate_prompt_sidecars(
            prompt.prompt_token_ids,
            prompt.prompt_sidecars,
            prompt.required_domains,
            prompt.expected_sidecars,
            context=f"prompt {prompt.id!r}",
        )
        _validate_expected_sidecars(
            prompt.required_domains,
            prompt.expected_sidecars,
            context=f"prompt {prompt.id!r}",
        )
    except ValueError as exc:
        return _failure("sidecar_structure_failed", str(exc))

    domain_ids = [int(value) for value in prompt.prompt_sidecars["token_domain_ids"]]
    required_domain_ids = set(
        _prompt_domain_id(name, context=f"prompt {prompt.id!r}")
        for name in prompt.required_domains
    )
    diagnostic_edges = prompt.prompt_sidecars.get("token_diagnostic_edges", ())
    cross_edges = prompt.prompt_sidecars.get("token_cross_domain_edges", ())
    if any(
        DomainKind(int(domain_id)) in _DIAGNOSTIC_DOMAINS
        for domain_id in required_domain_ids
    ) and not diagnostic_edges:
        return _failure(
            "sidecar_structure_failed",
            "diagnostic required domain has no token_diagnostic_edges",
        )
    if len(required_domain_ids) > 1:
        if not cross_edges:
            return _failure(
                "sidecar_structure_failed",
                "multiple required domains need token_cross_domain_edges",
            )
        if not any(
            domain_ids[int(edge["from"])] != domain_ids[int(edge["to"])]
            for edge in cross_edges
        ):
            return _failure(
                "sidecar_structure_failed",
                "token_cross_domain_edges must connect distinct domain ids",
            )
    receipt = prompt.sidecar_receipt()
    return {
        "status": "sidecar_structure_passed",
        "required_domains": list(prompt.required_domains),
        "cross_domain_edges": len(cross_edges),
        "diagnostic_edges": len(diagnostic_edges),
        "prompt_sidecar_receipt": receipt,
    }


def run_prompt_oracle(
    prompt: DomainEvalPrompt,
    completion: str,
    *,
    tool_overrides: ToolOverrides | None = None,
) -> dict[str, Any]:
    if prompt.oracle_kind == ORACLE_CPP_COMPILE_RUN:
        return compile_cpp_completion(
            prompt,
            completion,
            tool_overrides=tool_overrides,
        )
    if prompt.oracle_kind == ORACLE_SHELL_SYNTAX:
        return shell_syntax_oracle(
            prompt,
            completion,
            tool_overrides=tool_overrides,
        )
    if prompt.oracle_kind == ORACLE_BUILD_STRUCTURE:
        return build_structure_oracle(prompt, completion)
    if prompt.oracle_kind == ORACLE_DIAGNOSTIC_STRUCTURE:
        return diagnostic_structure_oracle(prompt, completion)
    if prompt.oracle_kind == ORACLE_SIDECAR_STRUCTURE:
        return sidecar_structure_oracle(prompt, completion)
    return _failure(
        "missing_compile_oracle",
        f"unknown oracle_kind {prompt.oracle_kind!r}",
    )


def evaluate(
    prompts: list[DomainEvalPrompt],
    completions: dict[str, str],
    *,
    compile: bool = True,
    tool_overrides: ToolOverrides | None = None,
) -> dict[str, Any]:
    rows: list[dict[str, Any]] = []
    for prompt in prompts:
        completion = completions.get(prompt.id)
        has_completion = completion is not None and bool(completion.strip())
        row: dict[str, Any] = {
            "id": prompt.id,
            "task_type": prompt.task_type,
            "oracle_kind": prompt.oracle_kind,
            "required_domains": list(prompt.required_domains),
            "expected_sidecars": list(prompt.expected_sidecars),
            "prompt_sidecar_receipt": prompt.sidecar_receipt(),
            "has_completion": has_completion,
        }
        if not has_completion:
            row["status"] = "missing_completion"
        else:
            assert completion is not None
            row["completion_chars"] = len(completion)
            if compile:
                row.update(
                    run_prompt_oracle(
                        prompt,
                        completion,
                        tool_overrides=tool_overrides,
                    )
                )
            else:
                row["status"] = "not_compiled"
        rows.append(row)
    status_counts = Counter(str(row.get("status")) for row in rows)
    known_statuses = PASS_STATUSES | FAIL_STATUSES
    unknown_statuses = set(status_counts) - known_statuses
    if unknown_statuses:
        raise RuntimeError(
            f"domain eval produced unknown statuses {sorted(unknown_statuses)}"
        )
    failed = sum(status_counts[status] for status in FAIL_STATUSES)
    report = {
        "prompts": len(prompts),
        "completion_rows": len(completions),
        "oracle_passed": sum(status_counts[status] for status in PASS_STATUSES),
        "failed": failed,
        "passed": failed == 0,
        "status_counts": dict(sorted(status_counts.items())),
        "rows": rows,
    }
    for status in sorted(known_statuses):
        report[status] = status_counts[status]
    return report


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--prompts", type=Path, default=Path("evals/domain_routed_prompts.jsonl")
    )
    parser.add_argument("--completions", type=Path)
    parser.add_argument(
        "--out", type=Path, default=Path("outputs/evals/domain_routed_codegen.json")
    )
    parser.add_argument("--no-compile", action="store_true")
    args = parser.parse_args()

    prompts = load_prompts(args.prompts)
    completions = load_completions(args.completions)
    report = evaluate(prompts, completions, compile=not args.no_compile)
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    print(
        json.dumps(
            {
                key: report[key]
                for key in (
                    "prompts",
                    "completion_rows",
                    "compile_passed",
                    "failed",
                    "passed",
                    "status_counts",
                )
            },
            sort_keys=True,
        )
    )
    return 0 if report["passed"] else 2


if __name__ == "__main__":
    raise SystemExit(main())
