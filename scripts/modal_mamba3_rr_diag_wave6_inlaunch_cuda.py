"""Modal H200 runner for the wave6 chunk-owner CUDA diagonal prototype."""
...
APP_NAME = "cppmega-mamba3-rr-diag-wave6-inlaunch-cuda"
...
def run_remote(
    ...
    path = bench_dir / "rr_diag_wave6_inlaunch_cuda.py"
    spec = importlib.util.spec_from_file_location("rr_diag_wave6_inlaunch_cuda", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"could not load {path}")
    mod = importlib.util.module_from_spec(spec)
    sys.modules["rr_diag_wave6_inlaunch_cuda"] = mod
...
def main(
    ...
    for shape in [part.strip() for part in shape_csv.split(",") if part.strip()]:
        result = run_remote.remote(shape, iters, warmup, torch_reference)
