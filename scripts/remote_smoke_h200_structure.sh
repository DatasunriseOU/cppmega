...
CPPMEGA_RUN_ID="${CPPMEGA_RUN_ID:-cppmega_structure_smoke}"
REMOTE_VENV="${REMOTE_VENV:-${REMOTE_ROOT}/cppmega-venv}"
REMOTE_LOG="${REMOTE_LOG:-${REMOTE_ROOT}/cppmega/${CPPMEGA_RUN_ID}.log}"
REMOTE_TMP_SCRIPT="${REMOTE_TMP_SCRIPT:-/tmp/cppmega-remote-structure-smoke.sh}"
LOCAL_TMP_SCRIPT="$(mktemp -t cppmega-remote-structure.XXXXXX.sh)"
...
patch_language_model_embedding_for_structure(
...
gcloud compute ssh "${REMOTE_HOST}" \
  --zone "${REMOTE_ZONE}" \
  --command "REMOTE_ROOT='${REMOTE_ROOT}' REMOTE_VENV='${REMOTE_VENV}' REMOTE_LOG='${REMOTE_LOG}' CPPMEGA_RUN_ID='${CPPMEGA_RUN_ID}' bash '${REMOTE_TMP_SCRIPT}' || (status=\$?; echo 'remote structure smoke failed; tail follows:'; tail -n 120 '${REMOTE_LOG}' || true; exit \$status)"
