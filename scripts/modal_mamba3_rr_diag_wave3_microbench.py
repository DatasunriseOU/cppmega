"""Modal H200 runner for the wave3 standalone R x R diagonal microbench."""
...
APP_NAME = "cppmega-mamba3-rr-diag-wave3-microbench"
...
def run_remote(shape: str, iters: int, warmup: int, num_warps: int) -> dict[str, Any]:
    ...
    path = bench_dir / "rr_diag_wave3_microbench.py"
    spec = importlib.util.spec_from_file_location("rr_diag_wave3_microbench", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"could not load {path}")
    mod = importlib.util.module_from_spec(spec)
    sys.modules["rr_diag_wave3_microbench"] = mod
...
def main(
    shape_csv: str = "representative,productionish",
    iters: int = 10,
    warmup: int = 3,
    num_warps: int = 4,
) -> None:
    ...
    for shape in [part.strip() for part in shape_csv.split(",") if part.strip()]:
        result = run_remote.remote(shape, iters, warmup, num_warps)
