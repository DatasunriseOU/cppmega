...
GroupedQuantizeProducer = Literal["single_output", "multi_output"]
...
class GroupedSplitQuantizeProfile:
    ...
    grouped_quantize_producer: GroupedQuantizeProducer = "single_output"
...
def _set_profile_env(profile: GroupedSplitQuantizeProfile) -> None:
    ...
    os.environ["CPPMEGA_TE_MXFP8_GROUPED_QUANTIZE_PRODUCER"] = (
        profile.grouped_quantize_producer
    )
...
def _run(profile: GroupedSplitQuantizeProfile) -> dict[str, Any]:
    ...
    te_capabilities = {
        "single_output_producer": hasattr(tex, "mxfp8_quantize_with_rowwise_transpose"),
        "multi_output_producer": hasattr(
            tex,
    ...
    return {
        ...
        "te_capabilities": te_capabilities,
        "shape": {
...
def _parse_args() -> GroupedSplitQuantizeProfile:
    ...
    parser.add_argument(
        "--grouped-quantize-producer",
        choices=("single_output", "multi_output"),
        default="single_output",
        help=(
            "single_output calls TE's rowwise-transpose producer once per split; "
            "multi_output requires a TE batched split producer and fails if the "
    ...
    return GroupedSplitQuantizeProfile(
        ...
        grouped_quantize_producer=args.grouped_quantize_producer,
    )
