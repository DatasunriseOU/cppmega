#!/usr/bin/env python3
"""Wave30 Lane D probe for a small Mamba3 bwd_bwd helper boundary.
...
This probe is H100-local only by policy for Wave30 Lane D. It refuses to build
or run on H200 and on non-H100 local devices.
"""
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("dpsiv_d_boundary", &dpsiv_d_boundary, "Wave30 dPsiV_D boundary helper");
}
"""
...
def _device_gate() -> DeviceGate:
    ...
    lowered = name.lower()
    if "h200" in lowered:
        return DeviceGate(False, "h200_forbidden_by_wave30_lane_d", name, capability)
    if capability != (9, 0) or "h100" not in lowered:
        return DeviceGate(False, "requires_local_h100_sm90", name, capability)
...
def _build_extension(verbose: bool) -> Any:
    ...
    return load_inline(
        name="mamba3_wave30_dpsiv_d_boundary_ext",
        cpp_sources=[CPP_SRC],
...
def _reference(
    ...
    qkd_t = qk_dot.transpose(-1, -2).float()
    qkd_contrib = torch.einsum("tcio,tcop->tcip", qkd_t, dphi_r)
    qkd_contrib = qkd_contrib * gamma[:, :, None, None]
    out = dpsiv + dphi.float() * d[:, None, :] + qkd_contrib.reshape(tiles, cs * r, p)
...
def _no_go_payload(gate: DeviceGate, args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "gate": asdict(gate),
        "policy": "Wave30 Lane D: H100/local only; H200 forbidden",
        "compiled": False,
        "ptxas": None,
        "timing_ms": None,
        "memory": None,
...
def main() -> int:
    ...
    if max_abs != 0.0:
        raise SystemExit(f"correctness failed: max_abs={max_abs}")
    ...
    payload = {
        ...
        "gate": asdict(gate),
        "compiled": True,
        "ptxas": "emitted by --verbose-build via -Xptxas=-v",
        "correctness": {"max_abs_vs_torch_bf16_reference": max_abs},
        "timing_ms": {"kernel": kernel_ms, "torch_reference": ref_ms},
        "memory": {
            "theoretical_bytes": _bytes(args),
