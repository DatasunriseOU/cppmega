...
def _set_mxfp8_profile_env(
    backend: str | None, linear_kernel_contract: str | None
) -> tuple[str, str]:
    if linear_kernel_contract == "compact_direct_v1" and backend not in (
        None,
        "cutlass_native",
    ):
        raise ValueError("compact_direct_v1 requires --backend cutlass_native")
    ...
    precision.mxfp8_linear_kernel_contract = linear_kernel_contract or "gemm_ready_v1"
    if precision.mxfp8_linear_kernel_contract == "compact_direct_v1":
    ...
    return (
        os.environ["CPPMEGA_TE_MXFP8_BWD_BACKEND"],
        os.environ["CPPMEGA_TE_MXFP8_LINEAR_KERNEL_CONTRACT"],
    )
...
def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    backend, linear_kernel_contract = _set_mxfp8_profile_env(
        args.backend, args.linear_kernel_contract
    )
    ...
    if direct_no_sidecar or flashinfer_compact_direct:
        if linear_kernel_contract == "compact_direct_v1" and not saved_input_columnwise_payload:
            failures.append(
                "compact_direct_v1 did not save an MXFP8 input columnwise payload"
            )
        ...
        if int(stats.get("mxfp8_tn_sidecar_registry_peak_bytes", 0)) != 0:
            failures.append("compact-direct backend used sidecar registry bytes")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Override the typed MXFP8 Linear backward contract before loading the shim.",
    )
