...
Added a guarded source applier for the Mamba3 MIMO backward GQA reduction
branch:
...
pytest -q \
  tests/test_mamba3_gqa_bwd_applier.py \
  tests/test_mamba3_stage2_force_nontma_applier.py

PYTHONPATH=. python -m cppmega.megatron.upstream_patches.apply_mamba3_gqa_bwd_patches

PYTHONPATH=. CPPMEGA_MAMBA3_GQA_BWD=1 \
...
field is an error even though the applier stdout confirms both GQA patches were
