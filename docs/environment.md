...
Latest local suite run from the shared venv, no extra environment variables:

```bash
cd /Volumes/external/sources/cppmega
.venv/bin/python -m pytest tests/ -q
# 2582 passed, 182 skipped, 0 failed
```

Receipt: `outputs/reports/local_suite_baseline_2026_08_01.json`:

```json
{
  "schema": 1,
  "report": "local_suite_baseline_2026_08_01",
  "repo": "cppmega",
  "commit": "131bf13b10e0e0165b739d091e3c218a7dcdcfff",
  "tree": "3e31974edd9908c0887b7a0a80dcec79b1338940",
  "timestamp": "2026-08-01T19:11:16.590720+00:00",
...
Cross-repository parity tests now `skip` instead of `fail` when the
reference checkout variables are absent, so the default command above is green
without `CPPMEGA_MLX_REFERENCE_ROOT` or `CPPMEGA_RECIPE_PARITY_PEER_ROOT`.
When those variables are set, the tests validate the exact commit and still
