...
PY
    ...
    CHECKPOINT_ARGS=()
    CHECKPOINT_ARGS+=(--load /data/cppmega_load_checkpoint)
    CHECKPOINT_ARGS+=(--no-load-optim --no-load-rng)
    CHECKPOINT_ARGS+=(--save-interval 50000000)
  ...
  if [[ "$status" != 0 ]]; then
    ...
    if grep -qE 'iteration[[:space:]]+5001/[[:space:]]+5001' "$LOG" && \
       grep -qE 'validation loss at iteration 5001' "$LOG" && \
