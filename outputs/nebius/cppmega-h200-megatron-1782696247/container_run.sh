        ...
        TEST_SPECS=(
          1024:cppmega_1024_current_mix_graph_train
        )
...
CHECKPOINT_ARGS+=(--save \$CHECKPOINT_ROOT --save-interval 1000)
          ...
          if [[ "$status" != 0 ]]; then
            ...
            if grep -qE 'iteration[[:space:]]+5000/[[:space:]]+5000' "$LOG" && \
               grep -qE 'validation loss at iteration 5000' "$LOG" && \
        ...
        sudo docker run --gpus all --rm --ipc=host --ulimit memlock=-1 --ulimit stack=67108864           -v /data:/data           -v /data/cppmega_overlay:/overlay:ro           ghcr.io/datasunriseou/cppmega:latest           bash /data/cppmega_h200_results/container_run.sh
