...
PY
    ...
    if [[ \"\$CPPMEGA_FP8_RECIPE\" == tensorwise ]]; then
      # Current TE checkpoint path rejects Megatron's padding_mask kwarg
      # in FP8 MLP recompute. Keep the FP8 smoke on the real model/data
      # path and fail loudly on any other FP8 issue.
      RECOMPUTE_ARGS=()
    fi
  ...
  if [[ "$status" != 0 ]]; then
    ...
    if grep -qE 'iteration[[:space:]]+100/[[:space:]]+100' "$LOG" && \
       grep -qE 'validation loss at iteration 100' "$LOG" && \
       grep -q 'transformer_engine::rtc::Kernel::~Kernel' "$LOG" && \
       grep -q 'SIGSEGV' "$LOG"; then
      echo "CPPMEGA_BATCH_RESULT seq=${SEQ} batch=${BS} status=OK_TE_CLEANUP_SIGSEGV exit=${status}" | tee -a "$LOG" | tee -a /data/cppmega_h200_results/summary.log
      continue
    fi
