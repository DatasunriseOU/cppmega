...
for SPEC in "${STAGES[@]}"; do
  ...
  export DATA_PREFIX
  echo "CPPMEGA_CURRICULUM_STAGE_START stage=${STAGE_IDX} seq=${SEQ} batch=${BS} stage_iters=${STAGE_ITERS} target_iter=${TARGET_ITERS} cumulative_after=${CUM_ITERS} prefix=${DATA_PREFIX}" | tee "$LOG" | tee -a /data/cppmega_h200_results/summary.log
