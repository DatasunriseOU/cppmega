...
TEST_SPECS=(
  1024:cppmega_reindexed_seq1024_lossmask_graph_train
)
...
PY
    ...
    CHECKPOINT_ARGS+=(--save \$CHECKPOINT_ROOT --save-interval 1421)
  ...
  if [[ "$status" != 0 ]]; then
    ...
    if grep -qE 'iteration[[:space:]]+1421/[[:space:]]+1421' "$LOG" && \
       grep -qE 'validation loss at iteration 1421' "$LOG" && \
