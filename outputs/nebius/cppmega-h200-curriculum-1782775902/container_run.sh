...
STAGES=(
          1:1024:192:1421:cppmega_reindexed_seq1024_lossmask_graph_train:/data/cppmega_curriculum_checkpoints/stage_01_seq_1024_bs_192
  2:2048:96:1686:cppmega_reindexed_seq2048_lossmask_graph_train:/data/cppmega_curriculum_checkpoints/stage_02_seq_2048_bs_96
  3:4096:48:1926:cppmega_reindexed_seq4096_lossmask_graph_train:/data/cppmega_curriculum_checkpoints/stage_03_seq_4096_bs_48
  4:8192:12:3674:cppmega_reindexed_seq8192_lossmask_graph_train:/data/cppmega_curriculum_checkpoints/stage_04_seq_8192_bs_12
  5:16384:3:6376:cppmega_reindexed_seq16384_lossmask_graph_train:/data/cppmega_curriculum_checkpoints/stage_05_seq_16384_bs_3
)

PREV_CHECKPOINT_ROOT=""
PREV_CUM_ITERS=0
for SPEC in "${STAGES[@]}"; do
  IFS=: read -r STAGE_IDX SEQ BS STAGE_ITERS DATA_PREFIX_NAME CHECKPOINT_ROOT <<< "$SPEC"
  CUM_ITERS=$((PREV_CUM_ITERS + STAGE_ITERS))
  DATA_PREFIX="/data/cppmega_sidecar/${DATA_PREFIX_NAME}"
  LOG="/data/cppmega_h200_results/stage_${STAGE_IDX}_seq_${SEQ}_bs_${BS}.log"
  NVSMI="/data/cppmega_h200_results/stage_${STAGE_IDX}_seq_${SEQ}_bs_${BS}.nvsmi.csv"
  export DATA_PREFIX
  echo "CPPMEGA_CURRICULUM_STAGE_START stage=${STAGE_IDX} seq=${SEQ} batch=${BS} stage_iters=${STAGE_ITERS} target_iter=${CUM_ITERS} prefix=${DATA_PREFIX}" | tee "$LOG" | tee -a /data/cppmega_h200_results/summary.log
...
PY
    ...
    CHECKPOINT_ARGS=(--save \"$CHECKPOINT_ROOT\" --save-interval ${CUM_ITERS})
    if [[ -n \"$PREV_CHECKPOINT_ROOT\" ]]; then
      CHECKPOINT_ARGS+=(--load \"$PREV_CHECKPOINT_ROOT\")
    fi
  ...
  echo "CPPMEGA_NVIDIA_SMI_PEAK stage=${STAGE_IDX} seq=${SEQ} batch=${BS} peak_used_mib=${peak}" | tee -a "$LOG" | tee -a /data/cppmega_h200_results/summary.log
  if [[ "$status" != 0 ]]; then
    echo "CPPMEGA_CURRICULUM_STAGE_RESULT stage=${STAGE_IDX} seq=${SEQ} batch=${BS} status=FAIL exit=${status}" | tee -a "$LOG" | tee -a /data/cppmega_h200_results/summary.log
    exit "$status"
  fi
  ...
  echo "CPPMEGA_CURRICULUM_STAGE_RESULT stage=${STAGE_IDX} seq=${SEQ} batch=${BS} status=OK checkpoint_root=${CHECKPOINT_ROOT}" | tee -a "$LOG" | tee -a /data/cppmega_h200_results/summary.log
